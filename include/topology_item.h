...
*/
// ---
class MbLabel : public MbSyncItem {
...
public:
  ...
  MbLabel( const MbeLabelState lbl );
  /// \ru Конструктор по Label. \en Constructor by Label. 
  MbLabel( const MbLabel & lbl );
  ...
  void SetLabel( const MbeLabelState lbl, void * key = NULL );
  ...
  void operator = ( const MbLabel & lbl );
...
private:
  bool operator == ( const MbLabel & lbl ); // \ru Не реализовано!!! \en Not implemented!!! 
};
...
public:
  ...
  virtual MbeTopologyType IsA() const = 0; 
          ...
          /// \ru Это вершина? \en Is it a vertex? 
          bool        IsAVertex() const { return  IsA()==tt_Vertex;    } 
          /// \ru Это ребро? \en Is it an edge? 
          bool        IsAEdge()   const { return (IsA()==tt_CurveEdge || IsA()==tt_OrientedEdge); } 
          /// \ru Это грань? \en Is it a face? 
          bool        IsAFace()   const { return  IsA()==tt_Face;      } 
          /// \ru Это оболочка? \en Is it a shell? 
          bool        IsAShell()  const { return  IsA()==tt_FaceShell; } 

private:
          // \ru Объявление оператора присваивания без реализации, чтобы не было присваивания по умолчанию. \en The declaration of the assignment operator without implementation to prevent an assignment by default. 
          MbTopItem & operator = ( const MbTopItem & ); // \ru Не реализовано!!! \en Not implemented!!! 
...
private:
  ...
  MbTopologyItem( const MbTopologyItem & ); 
protected:
  /// \ru Конструктор без параметров. \en Constructor without parameters. 
  MbTopologyItem(); 
  /// \ru Конструктор дублирования. \en Constructor of duplicating. 
...
private:
          // \ru Объявление оператора присваивания без реализации, чтобы не было присваивания по умолчанию. \en The declaration of the assignment operator without implementation to prevent an assignment by default. 
          void        operator = ( const MbTopologyItem & ); 
