...
private:
  ...
  cdet_app_item _Parent( cdet_app_item appItem ) const
  {
    ...
    PRECONDITION( iter->second < iter->first );
...
//------------------------------------------------------------------------------
// It will de removed from the API
// ---
class MbCompCollisionFaceArr : public SSArray<MbCollisionFace> 
{
  const void * component;

public:
  MbCompCollisionFaceArr( const void * comp )
    : SSArray<MbCollisionFace>( 0, 2 )
    , component( NULL )
  {
    component = comp;
  }
  MbCompCollisionFaceArr( const MbCompCollisionFaceArr & other )
    : SSArray<MbCollisionFace>( 0, 2 )
    , component( NULL )
  {
    Assign(other);
  }
  const void *      GetComponent() const { return component; }
  void              Assign( const MbCompCollisionFaceArr & other );
  MATH_FUNC (bool)  Init( MbCompCollisionFaceArr & other, MbCompCollisionFaceArr & detached );

  MbCompCollisionFaceArr & operator =   ( const MbCompCollisionFaceArr & other ) { Assign(other); return *this; }
  bool                     operator >   ( const MbCompCollisionFaceArr & other ) const { return component >  other.component; }
  bool                     operator <   ( const MbCompCollisionFaceArr & other ) const { return component <  other.component; }
  bool                     operator ==  ( const MbCompCollisionFaceArr & other ) const { return component == other.component; }
  bool                     operator !=  ( const MbCompCollisionFaceArr & other ) const { return !(*this   == other);          }

protected:
  void  AddOther( const MbCompCollisionFaceArr & other )
  { 
    for ( size_t k = 0, kcnt = other.Count(); k < kcnt; ++k )
      AddSimple( other[k] );
  }
};
...
//
// ---
inline void MbCompCollisionFaceArr::Assign( const MbCompCollisionFaceArr & other ) 
{
  Flush();
...
// ---
class MbProximityParameters 
{
public:
  MbCollisionFace * theFace1;     //
  MbCollisionFace * theFace2;     //
  MbCartPoint       thePar1;      // \ru Параметр близости на первой грани. \en Proximity parameter on the first face. 
  MbCartPoint       thePar2;      // \ru Параметр близости на второй грани. \en Proximity parameter on the second face. 
...
public:
  ...
  void  SetFacePair( const MbFace &topoFace1, const MbFace &topoFace2 );
...
// ---
class MbCollisionFaceArr: public cdet_query 
{
  typedef MbCompCollisionFaceArr comp_faces;
  SPArray<comp_faces>            compFaces;
  MbCompCollisionFaceArr *       lastComp;

  MbCollisionFaceArr();
  void Add( const void * comp, const RPArray<const MbFace> & );
  void Flush();
};
