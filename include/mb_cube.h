...
public :
  ...
  */
  bool      Intersect( const MbCube & other, double eps = (MIN_RADIUS + MIN_RADIUS) ) const; 
  ...
  /** 
    ...
    \return \ru true, если пересекается.
            \en Returns true if it intersects. \~
  */
  bool      Intersect( const MbPlacement3D & pl, double eps = MIN_RADIUS ) const;
  ...
  */
  ...
  /**
    ...
    \return \ru true, если пересечение есть, \n false в противном случае.
            \en Returns true if intersection exists, \n false otherwise. \~
  */
  bool      Intersection( const MbCube & cube1, const MbCube & cube2, double eps = MIN_RADIUS );
  ...
  */
  ...
  /// \ru Расширить куб во все стороны на соответсвующую величину. \en Expand the box in all directions on a corresponding amount. 
...
{
  return ::EqualPoints( pmin, other.pmin, Math::lengthEpsilon ) &&
         ::EqualPoints( pmax, other.pmax, Math::lengthEpsilon );
}
...
*/
// ---
inline bool EqualCubes( const MbCube & c1, const MbCube & c2, double eps = METRIC_EPSILON )
{
  bool bEqual = false;
  if ( !c1.IsEmpty() && !c2.IsEmpty() ) {
    bEqual = ( ::EqualPoints( c1.pmin, c2.pmin, eps ) &&
               ::EqualPoints( c1.pmax, c2.pmax, eps ) );
  }
  return bEqual;
}
