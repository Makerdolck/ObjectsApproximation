...
FORVARD_DECL_TEMPLATE_TYPENAME( void    set_Rarray_size( RPArray<Type> &, size_t newSize ) );
...
public:
  ...
  /// \ru Установить максимальное из приращений. \en Set the maximum increment. 
          ...
          void    SetSize ( size_t newSize );                           ///< \ru Установить новый размер массива. \en Set the new size of an array. 
          ...
          void    Add     ( Type * );                 ///< \ru Добавить элемент в конец массива. \en Add an element to the end of the array. 
          void    AddAt   ( Type * e, size_t index ) { Insert( index, e ); } ///< \ru Вставить элемент в указанную позицию. \en Insert an element to the given position. 
          void    AddAfter( Type * e, size_t index ); ///< \ru Добавить элемент после указанного. \en Add an element after the specified one. 
          void    Insert  ( size_t index, Type * );   ///< \ru Вставить элемент перед указанным. \en Insert an element before the specified one. 

          void    AddArray   ( const RPArray<Type> & );               ///< \ru Добавить массив. \en Add array. 
          void    AddCArray  ( const Type **, size_t count );         ///< \ru Добавить C-массив. \en Add C-array. 
          void    InsertArray( const RPArray<Type> &, size_t index ); ///< \ru Добавить массив в позицию. \en Add an array to the position. 
...
protected:
  ...
  const   TPtr * _End()     const { return parr + count; } ///< \ru Получить указатель на участок памяти после массива. \en Get the pointer to the piece of memory after the array. 
          ...
          void    AddMemory( size_t n ); ///< \ru Обеспечить место в памяти под n элементов, независимо от AutoDelta \en Provide memory for n elements, independently from AutoDelta 
...
private:
  ...
  TEMPLATE_FRIEND void    set_Rarray_size TEMPLATE_SUFFIX ( RPArray<Type> &, size_t newSize );
...
// ---
template <class Type>
inline void RPArray<Type>::SetSize( size_t newSize ) {
  set_Rarray_size( *this, newSize );
}
...
// ---
template <class Type>
inline void RPArray<Type>::Reserve( size_t n, bool addAdditionalSpace ) {
  if ( addAdditionalSpace ) {
    size_t nn = count + n;
    if ( upper < nn ) 
      set_Rarray_size( *this, nn );
  }
  else {
...
// ---
template <class Type>
inline void RPArray<Type>::AddMemory( size_t n ) {
  if ( upper - count < n )
    set_Rarray_size( *this, count + n );
}
...
// ---
template <class Type>
inline void RPArray<Type>::Add( Type * ent ) {
  CatchMemory();
  parr[count++] = ent;
}
...
// ---
template <class Type>
inline void RPArray<Type>::AddAfter( Type * ent, size_t  index ) {
  if ( !count )
    Add( ent );
  else {
...
// \ru Tеперь об этом нужно заботиться самостоятельно ! \en Now you should take care of it ! 
// ---
template <class Type>
inline void RPArray<Type>::AddArray( const RPArray<Type> & from ) {
  AddCArray( from.GetAddr(), from.count );
}
...
// ---
template <class Type>
inline void RPArray<Type>::AddCArray( const Type ** from, size_t fromCount ) {
  if ( fromCount ) {
    AddMemory( fromCount );  // \ru обеспечить память на такое кол-во элементов \en allocate memory for the given number of elements 
...
// ---
template <class Type>
inline void RPArray<Type>::InsertArray( const RPArray<Type> & from, size_t index ) {
  if ( from.count ) {
    if ( index > count )
      index = count;
    AddMemory( from.count );
...
// ---
template <class Type>
inline void RPArray<Type>::Insert( size_t index, Type * ent ) {
  CatchMemory();         // \ru добавить памяти, если все использовано \en add memory if whole allocated memory is used 
	if ( index >= count )
    index = count;
  else {
...
template <class Type>
template <class InputIterator>
void RPArray<Type>::insert( InputIterator pos, const Type * e )
{
  if ( !begin() ) {
    reserve( 1 );
    pos = begin();
...
template <class Type>
template <class InputIterator>
void RPArray<Type>::erase( InputIterator pos )
{
  if ( begin() ) {
    const ptrdiff_t k = std::distance( (InputIterator)begin(), pos );
    if ( k >= 0 && k < count )
...
// ---
template <class Type>
inline void RPArray<Type>::CatchMemory() {
	if ( upper == count )
    set_Rarray_size( *this, upper + AutoDelta() );
}
...
// ---
template <class Type>
void set_Rarray_size( RPArray<Type> & arr, size_t newSize )
{
