...
protected :
  std::vector<MbEdge *> edges;       ///< \ru Множество рёбер каркаса. \en A set of edges of the frame. 
...
public :
  ...
  virtual bool    GetPlacement( MbPlacement3D & place ) const; // \ru Проинициализировать присланную локальную систему координат (совместить плоскость XY), если каркас плоский. \en Initialize the sent local coordinate system (combine the plane XY) if the frame is planar. 
  ...
  const   MbEdge * GetEdge( size_t index ) const; 
          ...
          MbEdge * FindEdgeByName  ( const MbName & ) const; 
...
{
  if ( wireFrame != NULL ) {
    std::vector< SPtr<MbCurve3D> > curvesVect;
