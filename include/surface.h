...
public:
  ...
  */
  ...
  virtual void    CalculateWire( const MbStepData & stepData, MbMesh & mesh ) const; // \ru Вычислить полигональную сетку. \en Calculate polygonal mesh.  
  ...
  */
  ...
  /** \brief \ru Скопировать габаритный куб из копии.
    ...
    \param[in] s - \ru Поверхность-копия.
                   \en A surface-copy. \~
  */
          void    CopyGabarit( const MbSurface & s ) { cube = s.cube; }
  ...
  */
  ...
  /// \ru Установить расширенные пределы поверхности. Для внутреннеого использования. \en Set extended limits of surface. For internal use only. 
  virtual void    SetExtendedParamRegion( double u1, double v1, double u2, double v2 );
  /// \ru Включить точку в область определения. Для внутреннеого использования. \en Include a point into domain. For internal use only. 
  ...
  /// \ru Дать максимальное приращение параметра U. \en Get the maximum increment of U-parameter. 
          double  GetMaxParamDeltaU() const { return (GetUMax() - GetUMin()) / COUNT_DELTA; }
  /// \ru Дать максимальное приращение параметра V. \en Get the maximum increment of V-parameter. 
          double  GetMaxParamDeltaV() const { return (GetVMax() - GetVMin()) / COUNT_DELTA; }
  ...
  */
  ...
  /** \brief \ru Рассчитать сетку.
             \en Calculate mesh. \~
    \details \ru В функции производится рассчет сетки для отрисовки поверхности.
  ...
  */
  virtual void    CalculateSurfaceWire( const MbStepData & stepData, size_t beg, MbMesh & mesh, 
                                        size_t uMeshCount = WIRE_MAX, size_t vMeshCount = WIRE_MAX ) const;
  ...
  */
  ...
  /** \brief \ru Аппроксимировать поверхность треугольными пластинами.
             \en Approximate a surface by triangular plates. \~
    \details \ru Аппроксимировать поверхность треугольными пластинами. Аппроксимируется лицевая сторона поверхности.
             \en Approximate a surface by triangular plates. The front side of a surface is approximated. \~
    \param[in] stepData - \ru Данные для вычисления шага при триангуляции.
                          \en Data for step calculation during triangulation. \~
    \param[in] note     - \ru Способ построения полигонального объекта.
                          \en Way for polygonal object constructing. \~
    \param[out] mesh - \ru Результат разбиения.
                       \en The result of splitting. \~
  */
  ...
  */
  virtual bool    IsShift( const MbSpaceItem & obj, MbVector3D & dir, bool & isSame ) const;
...
protected:
  ...
  /// \ru Вычислить нормаль по известным производнымв uDer и vDer точке с параметрами u, v. \en Normal calculation by derivatives uDer and vDer on point with parameters u, v.
