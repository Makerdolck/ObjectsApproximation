...
public:
  ...
  struct Pair
  {
    const KeyType m_key;
    const ValType m_val;
    Pair( KeyType key, ValType val ) : m_key( key ), m_val( val ) {}    
...
// ---
template<class KeyType, class ValType >
inline void MultiMap<KeyType,ValType>::Associate( KeyType key, const ValType val ) {
  Pair pair( key, val );
  std::pair<size_t, size_t> idx = EqualRangeEx ( m_Pairs, key );
  if ( idx.second < m_Pairs.Count() )
...
// ---
template<class KeyType, class ValType >
inline void MultiMap<KeyType,ValType>::Dissociate( KeyType key, const ValType val ) {
  Pair pair( key, val );
...
// ---
template<class KeyType, class ValType >
inline typename MultiMap<KeyType,ValType>::Iterator MultiMap<KeyType,ValType>::operator[] ( KeyType key ) const {
