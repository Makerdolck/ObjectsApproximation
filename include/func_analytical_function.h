...
class MbMathematicalNode; // \ru Математический узел \en The mathermatical node 
...
*/
// ---
class MATH_CLASS MdCharacterFunction : public MbFunction {
private :
  const MbMathematicalNode * expression; ///< \ru Выражение функции в виде дерева. \en An expression of a function as tree. 
  const MbListVars *         variables;  ///< \ru Аргументы фукнции в виде списка. \en Arguments of a function as list 
...
private:
  ...
  bool           sense; ///< \ru Направление. \en Direction. 
private:
  mutable double tCur;               // \ru Параметр для которого рассчитаны производные \en The parameter where the derivatives have been calculated 
  mutable double derivatives[cdt_CountDer]; // \ru Рассчитанные значения производных \en The calculated values of derivatives 
