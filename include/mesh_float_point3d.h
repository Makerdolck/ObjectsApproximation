...
public:
  ...
  */
  ...
  float & operator[](size_t i)       { PRECONDITION( i < 3 ); return (&x)[i]; }
  /// \ru Выдать координату по её номеру. \en Get coordinate by its index. 
  float   operator[](size_t i) const { PRECONDITION( i < 3 ); return (&x)[i]; }
...
inline MbFloatPoint3D operator / ( const MbFloatPoint3D & pnt, float factor )
{
  ...
  PRECONDITION( ::fabs(factor) > NULL_EPSILON );
...
//------------------------------------------------------------------------------
// \ru Равенство точек \en Equality of points 
// ---
inline bool EqualPoints( const MbFloatPoint3D & p1, const MbFloatPoint3D & p2, double eps ) {
...
inline MbFloatVector3D & MbFloatVector3D::operator /= ( float f )
{
  PRECONDITION( ::fabs(f) > NULL_EPSILON );
  f = (float)( 1.0 / f );
  x *= f; y *= f; z *= f; 
  return *this;
}
...
// \ru Равенство точек \en Equality of points 
// ---
inline bool EqualVectors( const MbFloatVector3D &p1, const MbFloatVector3D &p2, double eps ) {
  return p1.IsSame( p2, eps );
}
