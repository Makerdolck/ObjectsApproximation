...
/**
  \file
  \brief \ru Класс GLSurfaceFormat представляет собой формат GLSurface.
         \en GLSurfaceFormat class represents GLSurface format. \~
...
*/
// ---
class VSN_CLASS GLSurfaceFormat
{
public:
  enum FormatOption 
  {
...
protected:
  VSN_DECLARE_PRIVATE(GLSurfaceFormat)

private:
  friend VSN_FUNC_EX bool operator==(const GLSurfaceFormat&, const GLSurfaceFormat&);
  friend VSN_FUNC_EX bool operator!=(const GLSurfaceFormat&, const GLSurfaceFormat&);
};

// \ru Вернуть true, если варианты двух объектов формата GLSurface равны.  \en True if options of two objects in GLSurface format are equal.
VSN_FUNC_EX bool operator==(const GLSurfaceFormat&, const GLSurfaceFormat&);
// \ru Вернуть false, если все варианты двух объектов формата GLSurface не равны, в противном случае возвращает true. \en False if all options of two objects in GLSurface format are not equal.
VSN_FUNC_EX bool operator!=(const GLSurfaceFormat&, const GLSurfaceFormat&);
