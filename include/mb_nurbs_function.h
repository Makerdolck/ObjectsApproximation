...
public:
  ...
  MbNurbsParameters( size_t d, size_t c, double zmin, double zmax, bool approx )
    ...
    , useApprox  ( approx )
  {
    PRECONDITION( d > 1 && d < SYS_MAX_UINT16 );
                            ...
                            const KnotsVector & knots )
{
  ...
  if ( res ) {
    if ( !::IsMonotonic( knots, true, true ) )
...
{
  PRECONDITION( (P != NULL && W != NULL) != (PW != NULL) );
  ...
  for ( k = 1; k <= d; k++ ) {
    ...
    for ( i = 0, icount = (r - k); i <= icount; i++ ) {
      r1i = ( r1 + i );
      PRECONDITION( ( ::fabs( U[r1i + degree] - U[r1i + k] ) > DOUBLE_EPSILON ) ); // \ru Проверка на сбой \en Check of failure 
                                  ...
                                  DoubleVector2 & lrVect )
{
  ...
  if ( (degree > 1) && ((ptrdiff_t)knots.size() > (ptrdiff_t)degree) ) {
    ...
    PRECONDITION( (size_t)begInd != SYS_MAX_T );
  }
                                    ...
                                    PointVector & pointList )
{
  ...
  if ( pointsCount > 1 && degree > 1 && knots.size() > 1 ) {
    ...
    if ( closed && (uppIndex > 1) &&
         ::EqualPoints( pointList[0], pointList[uppIndex], METRIC_REGION ) ) {
...
/*#if defined(C3D_DEBUG)
    if ( res ){ // \ru Отладка \en Debugging
      ...
      PRECONDITION( tang.Colinear( vect ) && tang * vect > ANGLE_EPSILON );
      PRECONDITION( !isC1 || ::fabs(tangLen - vectLen) < METRIC_EPSILON );
    }
#endif
*/
  }
...
/*#if defined(C3D_DEBUG)
  if ( res ) { // \ru Отладка \en Debugging
    ...
    PRECONDITION( vect.Colinear( tang ) );

    MbVector3D normal;
    nurbs._Normal( t, normal );
    PRECONDITION( normal.Colinear(tangDiff) );

    double curvature = tangDiff.Length();
    double curCurvature = nurbs.Curvature( t );
    PRECONDITION( ::fabs(curCurvature - curvature) < Math::metricAccuracy );
  }
#endif
*/
                             ...
                             DoubleVector & params ) 
{
  ...
  if ( isDone )
    ::SetLimitParam( params, 0.0, (double)((ptrdiff_t)points.size() - 1 + (cls ? 1 : 0)) );
...
size_t DefineApproxPointsClosed( Curve & curve, size_t pCount, double pmin, double pmax, ptrdiff_t degree, SArray<Point> & points, const SArray<double> & aKnots, const SArray<size_t> & pCounts )
{
  ...
  size_t stepCount = 0;
  double epsilon = curve.GetTRegion( METRIC_REGION );
  double t  = pmin;
  double angle = Math::deviateSag;
  SArray<double> tList( 0, 1 );
  SArray<double> restList( 0, 1 );

  size_t segmCount = curve.GetSegmentsCount();
  ptrdiff_t first_Segm = 0;
  ptrdiff_t last_Segm = 0;
  ...
  first_Segm = curve.FindSegment( pmin, tmin );
  last_Segm = curve.FindSegment( pmax, tmax );
  /*
  double crvLen = curve.CalculateLength( pmin, pmax );
  for ( i = first_Segm; i <= last_Segm; i++ ){
    ...
    if ( first_Segm == last_Segm ) {
      ...
      t1 = tmin;
      t2 = curve.GetSegment(i)->GetTMax();
      len = curve.GetSegment(i)->CalculateLength( t1, t2 );
    }
    else if ( i == last_Segm ) {
      t1 = curve.GetSegment(i)->GetTMin();
      t2 = tmax;
      len = curve.GetSegment(i)->CalculateLength( t1, t2 );
    }
    else 
      len = curve.GetSegment(i)->GetMetricLength();
    double part = len / crvLen;
  ...
  */
  ...
  for ( i = first_Segm; i <= last_Segm; i++ ) {
    ...
    while ( t < (smax - epsilon) ) {
      ...
      for ( size_t k = 0; k < tempCount; k++ ) 
        tList.Add( plusT + t + (step * factor * (double)k) );
      t += step;
      stepCount++;
    }
    tList.Add(plusT + smax);
    plusT += smax;
  ...
  if ( aKnots.Count() > 0 ) {
    ...
    ::SetLimitParam( pParams, aKnots[degree - 1], aKnots[aKnots.Count() - degree] );
    double t1, t2;
    bool bRes = false;
    while ( !bRes ) {
      bRes = true;
...
size_t DefineApproxPointsOpen( Curve & curve, size_t pCount, double pmin, double pmax, SArray<Point> & points, const SArray<double> & aKnots, const SArray<size_t> & pCounts )
{
  ...
  if ( pCounts.Count() != segmCount )
    return 0;

  double tpmin = pmin;
  double tpmax = pmax;

  first_Segm = curve.FindSegment( tpmin, tmin );
  last_Segm = curve.FindSegment( tpmax, tmax );
/*
  double crvLen = curve.CalculateLength( pmin, pmax );
  for ( i = first_Segm; i <= last_Segm; i++ ){
    double len = 0.0;
    ...
    if ( first_Segm == last_Segm ) {
      ...
      len = curve.GetSegment(i)->GetMetricLength();
    double part = len / crvLen;
    size_t segmPCnt = (size_t)((pCount - 1) * part);  //-V113
    if ( i == last_Segm )
      segmPCnt = freePntsCount;
    else {
      segmPCnt = std_max( segmPCnt, (size_t)1);
 ...
 */
  ...
  for ( i = first_Segm; i <= last_Segm; i++ ) {
    ...
    while ( t < (smax - epsilon) ) {
      ...
      for ( size_t j = 0; j < tempCount; j++ ) 
        tList.Add( plusT + t + (step * factor * (double)j) );
      t += step;
      stepCount++;
    }
    tList.Add(plusT + smax);
...
/*
  while ( t < (pmax - epsilon) ) {
    double step = curve.DeviationStep( t, angle );
    step = std_min( step, maxStep );
    if ( (t + step) >= (pmax - epsilon) ) 
      step = pmax - t;
    for ( size_t i = 0; i < pCount; i++ ) 
      tList.Add( t + (step * factor * (double)i) );
    t += step;
    stepCount++;
  }
  tList.Add( pmax );
  ...
  size_t iCount = tList.Count();
  pCountActual = pCount;
  SArray<double> restList( 0, 1 );

  for ( size_t i = 0; i < iCount; i += stepCount ) {
    t = tList[i];
    curve._PointOn ( t, p );
    points.Add ( p );
    restList.Add( t );
...
*/
  ...
  if ( aKnots.Count() > 0 ) {
    SArray<double> pParams( 0, 1 );
    ::CreateSplineParameters( points, spt_ChordLength, false, pParams );
    ::SetLimitParam( pParams, aKnots[0], aKnots[aKnots.MaxIndex()]);
    double t1, t2;
    bool bRes = false;
    while ( !bRes ) {
      bRes = true;
