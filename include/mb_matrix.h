...
public:
  ...
  /// \ru Доступ по ссылке к элементу матрицы. \en Access to a matrix element by a reference. 
        double &  operator() ( size_t i, size_t j )       { PRECONDITION( std_max( i, j ) < MATRIX_DIM_2D ); flag = MB_UNSET; return el[i][j]; }
  /// \ru Значение элемента матрицы. \en The value of a matrix element. 
  const double &  operator() ( size_t i, size_t j ) const { PRECONDITION( std_max( i, j ) < MATRIX_DIM_2D );                  return el[i][j]; }
...
*/
// ---
inline bool EqualMatrices( const MbMatrix & m1, const MbMatrix & m2, double accuracy ) {
