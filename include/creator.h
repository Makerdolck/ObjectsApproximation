...
/** \brief \ru Типы строителей.
  ...
  \details \ru Типы строителей оболочек тел, точечных и проволочных каркасов геометрической модели. \n
           \en Types of constructors of shells of solids, point-farmes and wire-frames of the geometric model. \n \~
...
enum MbeCreatorType {
  ...
  ct_JointBendSolid           = 611,  ///< \ru Строитель оболочки с комбинированным сгибом. \en Consructor of a shell with a composite bend. 
  ...
  ct_SimplifyFlatSolid        = 618,  ///< \ru Строитель упрощения развёртки литового тела. \en Constructor of the sheet solid flat pattern simplification.
...
protected :
  ...
  MbCreator( MbSNameMaker * n );
  /// \ru Конструктор по именователю. \en Constructor by name-maker. 
  MbCreator( const MbSNameMaker & n );
...
public :
  ...
  \details \ru Создать копию объекта с использованием регистратора. 
    ...
    which contain references to the different copies of a single embedded object, what leads to loss of relationsheep between the copies. \~
...
// ---
inline bool MbCreator::CreateShell( SPtr<MbFaceShell> & shell, MbeCopyMode sameShell,
                                    RPArray<MbSpaceItem> * items )
{
  MbFaceShell * shellPtr = shell;
  bool res = CreateShell( shellPtr, sameShell, items );
  shell = shellPtr;
  return res;
}
...
// \ru Построить оболочку по исходным данным. \en Create a shell from the initial data.
// ---
inline bool MbCreator::CreateWireFrame( SPtr<MbWireFrame> & frame, MbeCopyMode sameShell,
                                        RPArray<MbSpaceItem> * items )
{
  MbWireFrame * framePtr = frame;
  bool res = CreateWireFrame( framePtr, sameShell, items );
  frame = framePtr;
  return res;
}
...
// \ru Построить оболочку по исходным данным. \en Create a shell from the initial data.
// ---
inline bool MbCreator::CreatePointFrame( SPtr<MbPointFrame> & frame, MbeCopyMode sameShell,
                                         RPArray<MbSpaceItem> * items )
{
  MbPointFrame * framePtr = frame;
  bool res = CreatePointFrame( framePtr, sameShell, items );
  frame = framePtr;
  return res;
}
