...
public :
  ...
  virtual MbeImplicationType ImplicationType() const; 
          ...
          MbModel & Duplicate( MbRegDuplicate * iReg = NULL ) const; 
  ...
  /** \} */
          ...
          */
          ...
          void    DetachItems( RPArray<MbItem> & items ); 
          ...
          /** \brief \ru Наполнить присланную модель полигональными копиями объектов модели.
            ...
            \details \ru Наполнить присланную модель полигональными копиями объектов оригинальной модели.
              ...
              solids, wireframes, point-frames and other finite objects of model 
  ...
  */
  ...
  const   MbItem * GetItemByName( SimpleName n, MbPath & path, MbMatrix3D & from ) const; 
          ...
          */
          ...
          void    TransformSelected( const MbMatrix3D & matr, MbRegTransform * iReg = NULL ); 
          /// \ru Сдвинуть выбранные объекты. \en Move selected objects. 
          void    MoveSelected( const MbVector3D & to, MbRegTransform * iReg = NULL );
          /// \ru Повернуть выбранные объекты вокруг оси. \en Rotate selected objects around an axis. 
          void    RotateSelected( const MbAxis3D & axis, double angle, MbRegTransform * iReg = NULL );
          ...
          */
          void    DetachSelected( RPArray<MbItem> & items, SArray<MbMatrix3D> & matrs ); 
          ...
          void    AddModelItems( MbModel & mod );
          ...
          size_t  GetSelected( RPArray<MbItem> & objs ); 
...
//     ItemConstIterator( const ItemIterator& iIter ) : currIter( iIter.currIter ) {}
  ...
  public:
    const MbItem * operator *  () { return currIter->second; }
    const MbItem * operator -> () { return currIter->second; }
...
//----------------------------------------------------------------------------------------
/** \brief \ru Чтение из потока объектов, определенных в дереве модели.
           \en Read items, defined in the model tree, from a stream.
  \details \ru Чтение из потока объектов, определенных в дереве модели.
           \en Read items, defined in the model tree, from a stream.
...
*/ 
// ---
MATH_FUNC (void) ReadModelItemsFromTree( reader& in, c3d::IModelTree* tree, std::vector< SPtr<MbItem> >& items );
...
*/
// ---
MATH_FUNC (void) WriteModelItems( writer &, MbModel & );
