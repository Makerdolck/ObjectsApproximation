...
public :
  ...
  MbPolyline3D( const CcArray<MbCartPoint3D> & initList, ptrdiff_t count, bool cls, double scale ); // \ru Используется в конверторе парасолида \en Used in converter of parasolid 
protected :
...
public :
  VISITING_CLASS( MbPolyline3D ); 
          ...
          void    Init( const std::vector<MbCartPoint3D> & initList, bool cls );
