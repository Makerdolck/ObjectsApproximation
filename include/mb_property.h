...
public :
  ...
  /// \ru Конструктор. \en Constructor. 
  IntProperty( MbePrompt name, ptrdiff_t initValue, bool change = true )
    : MbProperty( name, change )
...
{
  PRECONDITION( value != NULL && v != NULL );
...
// ---
template <class PropType> 
inline void GetCharValue( const PropType *, class MbName * value, uint32 n, TCHAR * v ) 
{
  PRECONDITION( v != NULL );
  if ( v != NULL ) {
    if ( value != NULL ) {
