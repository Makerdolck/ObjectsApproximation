...
public :
  ...
  /// \ru Выставить состояние флага владения элементами. \en Set the flag of elements ownership 
          ...
          void    SetSize ( size_t newSize, bool clear ); ///< \ru Установить новый размер массива. \en Set the new size of an array. 
...
private:
  ...
  TEMPLATE_FRIEND void  set_Parray_size TEMPLATE_SUFFIX ( PArray<Type> &, size_t newSize, bool clear );
...
// ---
template <class Type>
inline void PArray<Type>::SetSize( size_t newSize, bool clear ) {
  set_Parray_size( *this, newSize, clear );
}
...
// \ru PRECONDITION( PArray<Type>::nowDeletedElem  == 0 ); // временно \en PRECONDITION( PArray<Type>::nowDeletedElem  == 0 ); // temporarily 
...
inline void PIArray<Type>::ForEachI( ParIteratorFunc func, void * pars ) const {
  PRECONDITION( PArray<Type>::nowDeletedElem  == 0 ); 
...
// \ru PRECONDITION( PArray<Type>::nowDeletedElem  == 0 ); 
//#if !defined ( __INTEL_COMPILER ) /// for Intel C++ Compiler
//  for_each_in_array( *this, func );
//#endif // __INTEL_COMPILER
//}
