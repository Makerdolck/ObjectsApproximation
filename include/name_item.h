...
/** \brief \ru Множество простых имен.
  ...
  \details \ru Множество содержи контейнер простых имен. \n
...
public:
  ...
  SimpleName *  GetAddr() const                                   { return (SimpleName *)m_array.GetAddr(); }
  ...
  /// \ru Получить элмент по индексу. \en Get element by an index. 
...
{ 
  ...
  for( size_t i = 0; i < count && out.good(); i++ )
    WriteSimpleName( out, (SimpleName&)ref.m_array[i] );
...
inline SimpleName Hash32( const SimpleNameArray & snArr ) { 
  return (SimpleName)::Hash32( (uint8*)snArr.GetAddr(), snArr.Count() * sizeof(SimpleName) );
}


//----------------------------------------------------------------------------------------
/// \ru Оператор конкатинации. \en Concatenation operator. 
// ---
...
public:
  ...
  SimpleName  Hash() const { if ( hash == SIMPLENAME_MAX ) hash = ::Hash32( (uint8*)parr, count * sizeof(SimpleName)/*4*/ ); return hash; } 
  void        FlashHash()  { hash = SIMPLENAME_MAX; } ///< \ru Cбросить свой хэш. \en Reset hash value.
  ...
  void        RemoveIndBase ( size_t delIndex ) { PRECONDITION(delIndex <  countBase); RemoveInd( delIndex ); countBase--; FlashHash(); }
  /// \ru Удалить дополнительный элемент из массива. \en Delete additional element from the array. 
  void        RemoveIndExtra( size_t delIndex ) { PRECONDITION(delIndex >= countBase); RemoveInd( delIndex ); FlashHash(); }
  /// \ru Получить основное простое имя по индексу. \en Get main simple name by an index. 
  const SimpleName & GetValueBase( size_t index ) const   { PRECONDITION(index<countBase); return parr[index]; }
  /// \ru Установить основное простое имя по индексу. \en Set main simple name by an index. 
  void        SetValueBase( SimpleName v, size_t index  ) { PRECONDITION(index<countBase); parr[index] = v; FlashHash(); }
  /// \ru Получить дополнительное простое имя по индексу. \en Get additional simple name by an index. 
  const SimpleName & GetValueExtra( size_t index ) const  { PRECONDITION(index>=countBase && index<count); return parr[index]; }
  /// \ru Установить дополнительное простое имя по индексу. \en Set additional simple name by an index. 
  void        SetValueExtra( SimpleName v, size_t index ) { PRECONDITION(index>=countBase && index<count); parr[index] = v; FlashHash(); }
...
inline size_t MbIdArr::SizeOf() const
{
  ...
  size += sizeof(LiSArray<SimpleName>) + Count() * sizeof(SimpleName); //-V119
  return size;
}
...
{ 
  PRECONDITION( CountAll() == CountBase() );
  if ( CountAll() == CountBase() ) {
    LiSArray<SimpleName>::operator += ( other );
    flags   = other.flags;

    PRECONDITION( (countBase + other.countBase) <= SYS_MAX_UINT16 ); // \ru Превышение размерности uint16 \en Exceeding of size of uint16 
...
#else
    typedef std::auto_ptr<MbName>   UniqueNamePtr;
#endif
...
public :
  ...
  enum ReservedMainNames
  {
    ...
    rmn_RightSectionName = -3, ///< \ru Системное имя правой секущей плоскости. \en Right section plane name.
    rmn_LeftSectionName  = -2, ///< \ru Системное имя левой  секущей плоскости. \en Left  section plane name.
...
public:
  ...
  SimpleName  GetFirstNameDirect()              const { PRECONDITION(defNames.CountBase() > (size_t)i_First); return defNames.GetValueBase((size_t)i_First);     }
  /// \ru Прямой доступ к первому имени. \en Direct access to the first name. 
  void        SetFirstNameDirect( SimpleName fi )     { PRECONDITION(defNames.CountBase() > (size_t)i_First);        defNames.SetValueBase(fi, (size_t)i_First); } 
...
public:
  ...
  void        MakeTemplate() { 
    PRECONDITION( defNames.CountBase() );
    if ( defNames.CountBase() > (size_t)i_First ) {
      ...
      defNames.AddBase( 0 );
    PRECONDITION( *this != *uniqueFaceName );
  }
...
{
  bool res = true;
  if ( n == static_cast<SimpleName>(rmn_DummyFaceName) ) { // Reserved for unique name of dummy face (-1 = SIMPLENAME_MAX, -2 is already used for Kompas CAD)
...
// ---
inline bool MbName::operator == ( const MbName & n2 ) const
{
  if ( defNames.CountAll() == n2.defNames.CountAll() ) {
    if ( defNames.CountAll() )
      return (::memcmp( defNames.GetAddr(), n2.defNames.GetAddr(), defNames.CountAll() * sizeof(SimpleName) ) == 0);
    return true;
  }
  return false;
}
...
inline bool MbName::operator < ( const MbName & n2 ) const
{
  ...
  else if ( defNames.CountAll() < n2.defNames.CountAll() )
    return true;
...
public:
  ...
  {
    PRECONDITION( (orig != NULL) && (copy != NULL) );
...
\ru Надо добиться того, чтобы имена не могли создаваться в обход определенных правил
  ...
  Если надо создать новое правило генерации имени понаследуйте от этого класса
...
protected:
  ...
  /// \ru Оператор записи. \en Write operator. 
  friend MATH_FUNC (writer &) operator << ( writer& out,       MbNameMaker & ref ) { return operator << ( out, (const MbNameMaker &)ref ); }
...
protected:
  ...
  /// \ru Оператор записи. \en Write operator. 
  friend MATH_FUNC (writer &) operator << ( writer & out,       MbSNameMaker & ref ) { return operator << ( out, (const MbSNameMaker &)ref ); }
...
inline MATH_FUNC (int) MbMemDefNameCompare( const MbName & n1, const MbName & n2 )
{
  ...
  if ( count1 == count2 ) {
    if ( count1 )
      res = ::memcmp( n1.defNames.GetAddr(), n2.defNames.GetAddr(), count1 * sizeof(SimpleName) ); 
    else
      res = 0;
  }
...
inline SimpleName Hash32( const SArray<SimpleName> & arr )  {
  return Hash32( (uint8*)arr.GetAddr(), arr.Count() * sizeof(SimpleName) );
}
