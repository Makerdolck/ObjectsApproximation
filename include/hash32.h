...
*/
// ---
inline bool IsGood( const SimpleName & s ) { return (bool)(s > 0); }


#else // SIMPLENAME_AS_CLASS 


// \ru KYA K13+ При активации этой ветки обязательно пересобрать проект с активацией максимального уровеня предупреждений \en KYA K13+ When this branch is activated, the project must be rebuilt with the highest level of warnings activated 

const size_t SIMPLENAME_MAX = SYS_MAX_T;
...
public:
  ...
  SimpleName()                          : body( SIMPLENAME_MAX ) {}
  SimpleName( const SimpleName& other ) : body( SIMPLENAME_MAX ) { body = other.body; }
  SimpleName( size_t other )            : body( SIMPLENAME_MAX ) { body = other; }
...
// \ru операторы сравнения \en compare operators 
  ...
  SimpleName &  operator *= ( size_t other )                   { body *= (size_t)other;      return *this; }
  SimpleName &  operator |= ( size_t other )                   { body |= (size_t)other;      return *this; }
	SimpleName &  operator ++ ()                                 { ++body;                     return *this; } // preincrement
	SimpleName    operator ++ ( int )                            { size_t _tmp = body; ++body; return _tmp;  } // postincrement

// \ru доступ к данным \en access to data 
        operator bool  () const { return body != 0 && body != SIMPLENAME_MAX; }
...
inline bool IsGood( const SimpleName & s ) { return (bool)s; }
...
*/
// ---
inline SimpleName HashStr( const char * c_str ) {
  PRECONDITION( c_str );
...
*/
// ---
inline SimpleName HashStr( const wchar_t *w_str ) {
  PRECONDITION( w_str );
...
{ 
  ...
  //OV_x64 return ( m_val > with.m_val ) ? 1 : ( ( m_val < with.m_val ) ? -1 : 0 );
  return ::SimpleNameCompare( m_val, with.m_val );
}


//------------------------------------------------------------------------------
// \ru Проверка на равенство char строкой \en Check for equatily of char by string 
//---
