...
public: /** \}
  ...
  /// \ru Найти угол с осью Z. \en Find angle to Z-axis. 
  double      Angle         ( const MbVector3D & v  ) const { return v.Angle(axisZ); }
  /// \ru Найти угол между вектором и плейсментом. \en Find the angle between a vector and the placement. 
  ...
  bool        IsShift ( const MbPlacement3D &, MbVector3D &, bool & ) const; 
...
inline bool MbPlacement3D::IsCircular() const
{
  if ( IsNormal() )
    ...
    if ( isOrthogonal ) {
      if ( ::EqualLengths( axisX.Length(), axisY.Length() ) )
        return true;
    }
  }
...
inline bool MbPlacement3D::IsIsotropic() const
{
  if ( IsNormal() )
    ...
    if ( ::EqualLengths( lx, ly ) ) {
      ...
      if ( ::EqualLengths( lz, lx ) && ::EqualLengths( lz, ly ) )
        return true;
    }
  }
  return false;
...
// ---
inline bool MbPlacement3D::IsShift( const MbPlacement3D & other, MbVector3D & vect, bool & isSame ) const
{
  if ( (axisZ == other.axisZ) &&
       (axisY == other.axisY) &&
       (axisX == other.axisX) ) {
