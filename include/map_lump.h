...
public:
  MbMapVisibilityMode( bool invis, bool skipVerts, bool shadeSpaceCurves = false ) : invisible( invis ), skipVertices( skipVerts ), shadeCurves( shadeSpaceCurves ) {}
  MbMapVisibilityMode( const MbMapVisibilityMode & m ) : invisible( m.invisible ), skipVertices( m.skipVertices ), shadeCurves( m.shadeCurves ) {}
public:
  void Init( bool invis, bool skipVerts, bool shadeSpaceCurves = false ) { invisible = invis; skipVertices = skipVerts; shadeCurves = shadeSpaceCurves;  }
...
public:
  MbMapVisibilityMode & operator = ( const MbMapVisibilityMode & m ) { invisible = m.invisible; skipVertices = m.skipVertices; shadeCurves = m.shadeCurves; return *this; }
private:
...
private:
  SPtr<const MbCurve3D>     curve;      ///< \ru Кривая. \en A curve. 
...
private:
  ...
  const MbSolid *           solid;      ///< \ru Тело, на котором нарезна резьба. \en A threaded solid. 
...
typedef std::multiset<MbLump*, LumpCompLess> SortLumpMultySet; 
...
private:
  /* \brief \ru Дать вид объекта.
             \en Get the object view. \~
    \details \ru Дать вид объекта.\n  
      Создает вид аннотационного объекта заданного типа,
      ассоциированный с топологическим объектом, определяемым переданным именем,
      и обязательный к отображению в определяемой системе координат.
             \en Get the object view.\n  
      Creates the view of an annotative object of specified type,
      associated with the topological object defined by given name
      and required to be mapped in the defined coordinate system. \~
    \param[in] objType - \ru Тип аннотированного объекта.
                         \en A type of an annotated object. \~
    \param[in] name    - \ru Имя грани, с которой ассоциирован объект.
                         \en A face name the object is associated with. \~
    \param[in] place   - \ru Видовая система координат.
                         \en A view coordinate system. \~
    \param[in] items   - \ru Множество интерфейсных указателей видов аннотированных объектов.
                         \en The array of a view interface pointers of an annotated objects. \~
    \return \ru true в случае успеха операции.
            \en returns true if the operation succeeded. \~
  */
  virtual bool GetObjectView(       AnnotatedObjectType  objType,
                              const MbName &             name,       
                              const MbPlacement3D &      place,       
                                    RPArray<MbRefItem> & items,
                                    bool                 draw ) = 0;  
public:
  /** \brief \ru Получить массив объектов.
    ...
    \param[in] threads    - \ru Имя грани, с которой ассоциирован объект.
                            \en A face name the object is associated with. \~       
  */
  virtual bool GetMathThreads( SortLumpMultySet &  bodies,RPArray<MbThread> & threads, bool draw ) = 0;
};
  ...
  /**\ru \name Функции изменения данных.
     \en \name Functions for changing data.
      \{ */
        ...
        */
        void          AdjustMemory() { points.shrink_to_fit(); names.shrink_to_fit(); }
...
private:
  ...
  MbMatrix3D            from;      ///< \ru Матрица пересчета в мир. \en A matrix of transformation to the world coordinate system. 
  MbName                name;      ///< \ru Имя набора, если есть. \en A name of set, if it exsists.  
  ...
  /**\ru \name Функции изменения данных.
     \en \name Functions for changing data.
      \{ */
        ...
        */
        void          AdjustMemory() { curves.shrink_to_fit(); names.shrink_to_fit(); }
...
// ---
inline void MbSpaceCurves::AddNamedCurve( MbCurve3D * crv, MbName * nm, bool noSameCheck )
{
...
private:
  ...
  std::vector< SPtr<MbLump> > * lumps; // \ru может быть нулем \en can be zero. 
