...
public :
  ...
  /// \ru Конструктор по оболочке и набору строителей, флагу копирования строителей и регистратору дублирования объектов. \en Constructor by shell and set of creators, flag of creators copying and registrator of objects duplicating. 
  MbSolid( MbFaceShell & shell, std::vector< SPtr<MbCreator> > & creators, bool sameCreators, MbRegDuplicate * iReg ); // BUG_40923
...
public :
  ...
  virtual void    GetBasisItems   ( RPArray<MbSpaceItem> & s ); // \ru Выдать базовые объекты. \en Get the base objects. 
  ...
  // \ru Достроить тело для последней невыполненной операции в журнале построения. \en Build a body by the last keeped operation in the build log.
  ...
  */
  virtual bool    GetCreators( RPArray<MbCreator> & creators ) const; 
          ...
          */
          ...
          MbVertex * FindNearestVertex( const MbCartPoint3D & p ) const; 
          ...
          */
          bool    SetMultiSolidState( bool ms ) const;
...
private:
  void  SetMultiSolidStateDirectly( bool ms ) { multiState = ms ? ms_Multiple : ms_Single; }
