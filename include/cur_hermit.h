...
public :
  VISITING_CLASS( MbHermit ); 
          ...
          void        Init( const SArray<MbCartPoint> & initPoints, bool cls );
          void        Init( const SArray<double> & initParams, 
                                       ...
                                       double    & quota1, double    & quota2 ) const
{
  ...
  if ( bmin || t > tmax ) { // \ru Параметр вне границ \en Parameter is out of bounds 
    if ( closed ) { 
      ...
      if ( ::fabs(tparam1) < DOUBLE_EPSILON ) {
        quota1 = 1.0;
        quota2 = 0.0;
      }
      else if ( ::fabs(tparam2) < DOUBLE_EPSILON ) {
        quota1 = 0.0;
        quota2 = 1.0;
      }
      else {
