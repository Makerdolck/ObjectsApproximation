...
inline wchar_t * mbsnewwcs( const char * str )
{
  ...
  if ( str )
  {
    ...
    else // \ru переводим по одному символу, вместо непереведенных ставим "?" \en convert by word and replace not converted words by the symbol "?" 
    {
      ...
      {
#ifdef _MSC_VER // \ru Актуально только под Windows см. Ошибка 42704 \en It is actual only for Windows Error 42704 
        wchar_t c;
...
#if defined (_MSC_VER) && !defined(ALL_WARNINGS)  // Set warnings level
#pragma warning(default: 4996)  // This function or variable may be unsafe. Consider using strcpy_s instead.
#endif
