...
protected :
  RPArray<MbCreator> creators;          ///< \ru Набор данных для построения исходных оболочек. \en Data set for construction of the source shells. 
  SArray<ptrdiff_t>  countNumbers;      ///< \ru Индекс начального строителя для следующей оболочки. \en Index of the source creator for the next shell. 
...
public :
  MbUnionSolid( const RPArray<MbCreator> & ss, bool same, SArray<ptrdiff_t> & countN, 
                OperationType op, bool check, bool mf, const MbSNameMaker & n );
private :
  MbUnionSolid( const MbUnionSolid & init, MbRegDuplicate *ireg );
  // \ru Объявление конструктора копирования без реализации, чтобы не было копирования по умолчанию. \en Declaration without implementation of the copy-constructor to prevent copying by default. 
  MbUnionSolid( const MbUnionSolid & init ); 
public :
  ...
  virtual void        SetYourVersion( VERSION version, bool forAll );
          ...
          OperationType GetOperationType() const { return operation; } // \ru Тип булевой операции над оболочками \en Type of Boolean operation on shells 
          double      GetBuildSag()      const { return buildSag; }
          size_t      GetCreatorsCount() const { return creators.Count(); }
          void        AddCreator ( MbCreator & creator ); // \ru Добавить в журнал построения. \en Add to the history tree. 
          MbCreator * GetCreator( size_t ind ) const; // \ru Дать строитель. \en Get the constructor. 
          void        DeleteCreator( size_t ind ); 
...
//------------------------------------------------------------------------------
/** \brief \ru Объединенить множества граней оболочек в одну оболчку.
...
*/
// ---
MATH_FUNC (MbCreator *) CreateUnion( const RPArray<MbCreator> & creators, 
                                     SArray<ptrdiff_t> & countNumbers, 
