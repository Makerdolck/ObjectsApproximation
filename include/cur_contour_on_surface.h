...
public :
  ...
  virtual void    CalculateWire( const MbStepData & stepData, MbMesh & ) const; // \ru Вычислить сетку. \en Calculate mesh. 
  ...
  virtual void    ChangeCarrier( MbSpaceItem & item, MbSpaceItem & init ); // \ru Изменить носителя. \en Change the carrier. 
  virtual bool    ChangeCarrierBorne( MbSpaceItem & item, MbSpaceItem & init, const MbMatrix & matr ); // \ru Изменить носимые элементы. \en Change carrier elements. 
  ...
  /// \ru Определить, находится ли контур cntr в области контура (известно, что контуры не пересекаются). \en Determine whether the contour "cntr" is inside the region of the contour (contours do not intersect). 
          MbeItemLocation ContourClassification( MbContourOnSurface & cntr, double precision = Math::metricPrecision ) const; 
