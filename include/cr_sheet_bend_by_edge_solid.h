...
class MATH_CLASS MbBendsByEdgesSolid : public MbCreator {
  ...
  PArray<MbSMBendNames>      bendsParams;  ///< \ru Множество параметров для каждого формируемого сгиба. \en Set of parameters for each bend. 
  
public :
  MbBendsByEdgesSolid( const SArray<MbEdgeFacesIndexes> & edgesIndices,
                       const bool                         unbended,
                       const MbBendByEdgeValues &         params,
                       const PArray<MbSMBendNames> &      bendsParams,
