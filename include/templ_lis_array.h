...
protected :
  ...
  // \ru ИР этот массив пусть прирастает всегда по 2 \en ИР let this array always increase by 2  
  enum {
    li_delta = 2,
  };
...
public:
  ...
  LiSArray<Type> &  operator += ( const LiSArray<Type> & );
  const Type &      operator []( size_t loc ) const { PRECONDITION(loc < count); return parr[loc]; } // \ru ИР PRECONDITION нужен для оценки выхода за пределы массива. Я один раз вышел полдня потом искал!!! \en ИР PRECONDITION is necessary to detect out of bounds. - 
        Type &      operator []( size_t loc )       { PRECONDITION(loc < count); return parr[loc]; } // \ru ИР PRECONDITION нужен для оценки выхода за пределы массива. Я один раз вышел полдня потом искал!!! \en ИР PRECONDITION is necessary to detect out of bounds. - 
...
{
  PRECONDITION( newSize <= 0xff ); // \ru поскльку здесь uint8 count \en since uint8 count 
  PRECONDITION( arr.count < 254 );
