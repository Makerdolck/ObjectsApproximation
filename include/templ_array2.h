...
FORVARD_DECL_TEMPLATE_TYPENAME( void     realloc_line ( Type *& line, size_t oldSize, size_t newSize ) );
...
FORVARD_DECL_TEMPLATE_TYPENAME( void     insert_column_to_array  ( Array2<Type> &, size_t ind ) );
FORVARD_DECL_TEMPLATE_TYPENAME( void     add_column_to_array     ( Array2<Type> & ) );
...
public: // Общие методы матриц (двумерных массивов)
   ...
   c3d::NumberPair      GetSize() const { return c3d::NumberPair( l, c ); } ///< \ru Дать размер массива. \en Give the size of the array. 
        bool            SetSize( c3d::NumberPair sz ) { return SetSize( sz.first, sz.second ); } ///< \ru Установить размер. \en Set size. 
...
public:
  ...
  const Type *          GetLine( size_t i = 0 ) const { PRECONDITION( i < l ); return parr[i]; }
        ...
        void            InsertColumn( size_t i = 0 ); ///< \ru Вставить столбец перед указанным. \en Insert column before the specified one. 
        void            AddColumn();                    ///< \ru Добавить столбец в конец массива. \en Add column to the end of the array. 
        void            RemoveColumn( size_t i = 0 ); ///< \ru Удалить столбец из массива. \en Delete column from array. 

        void            InsertLine  ( size_t i = 0 ); ///< \ru Вставить строку перед указанной. \en Insert row before the specified one. 
        void            AddLine();                      ///< \ru Добавить строку в конец массива. \en Add row to the end of the array. 
        void            RemoveLine  ( size_t i = 0 ); ///< \ru Удалить строку из массива. \en Delete row from array. 
protected :
        void            CatchLinePointers( size_t newCount ); ///< \ru Взять память под заданное кол-во указателей на строки. \en Allocate memory for the given count of pointers to rows. 
...
inline Array2<Type>::Array2( size_t lSize, size_t cSize )
  ...
  , c   ( 0 )
{
  ::set_array_size( *this, lSize, cSize );
...
{
  if ( lSize*cSize < MATRIX_MAX_COUNT )
    return new Array2<Type>( lSize, cSize );
  return NULL;
}
...
// ---
template <class Type>
inline void Array2<Type>::InsertColumn( size_t ind ) {
  insert_column_to_array( *this, ind );
}
...
// ---
template <class Type>
inline void Array2<Type>::AddColumn() {
  add_column_to_array( *this );
}
...
// ---
template <class Type>
inline void Array2<Type>::InsertLine( size_t ind )
{
  size_t oldL = l; // \ru прежнее кол-во строк \en previous count of rows 
  AddLine(); // \ru кол-во строк увеличилось на 1 \en count of rows is increased by 1 
...
// ---
template <class Type>
inline void Array2<Type>::AddLine()
{
  CatchLinePointers( l + 1 ); // \ru добавить память для одного указателя на строку \en add memory for one pointer to row 
  ...
  if ( c ) {
    realloc_line( newLine, 0/*oldSize*/, c/*newSize*/ ); // \ru захватить память под строку \en allocate memory for one row 
...
// ---
template <class Type>
inline void Array2<Type>::CatchLinePointers( size_t newCount )
{
  if ( l != newCount )
  {
...
// ---
template <class Type>
inline void realloc_line( Type *& line, size_t oldSize, size_t newSize )
{
  if ( oldSize != newSize )
  {
...
bool set_array_size( Array2<Type> & arr, size_t lSize, size_t cSize )
{
  ...
  if ( lSize*cSize <= MATRIX_MAX_COUNT ) {
    ...
    if ( arr.l != lSize ) {
      ...
      arr.CatchLinePointers( lSize );

      for ( i = oldL; i < arr.l/*\ru уже изменено \en already changed */; i++ )
    ...
    if ( arr.c != cSize ) {
      ...
      for ( i = 0; i < arr.l/*\ru уже изменено \en already changed */; i++ )
        realloc_line( arr.parr[i], arr.c/*oldSize*/, cSize/*newSize*/ );
      ...
      Type * newLine = 0; // \ru указатель на содержимое новой строки \en pointer to contents of new row 
      for ( i = oldL; i < arr.l; i++ ) {
        newLine = NULL;
        ::realloc_line( newLine, 0, arr.c ); // \ru захватить память под строку \en allocate memory for one row 
...
// ---
template <class Type>
void insert_column_to_array( Array2<Type> & arr, size_t ind )
{
  if ( arr.l ) {
    size_t oldC = arr.c; // \ru прежнее кол-во колонок \en previous count of columns 
    add_column_to_array( arr ); // \ru добавить колонку в конец строк (увеличилось arr.c) \en add column to the end of rows (arr.c increased) 
...
// ---
template <class Type>
void add_column_to_array( Array2<Type> & arr )
{
  if ( arr.l ) {
    // \ru увеличить память под каждой строкой на 1 элемент \en increase memory under each row by 1 element 
    size_t newSize  = arr.c + 1;
    for ( size_t i = 0; i < arr.l; i++ ) {
      realloc_line( arr.parr[i]/*\ru здесь может меняться!!! \en can be changed!!! */, arr.c/*oldSize*/, newSize );
