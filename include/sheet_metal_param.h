...
struct MATH_CLASS MbStampingValues {
  ...
  double bottomFilletRadius; ///< \ru Радиус скругления дна (отрицательное зачение запрещает скругление). \en Fillet radius of bottom (negative value prohibits fillet). 
...
public:
  ...
  bool IsSame( const MbAnyBend & other, double accuracy ) const {
    ...
    if ( ::EqualPoints( origin, other.origin, accuracy ) &&
         ::EqualVectors( vector, other.vector, accuracy ) &&
