...
/** \brief \ru Автоматически назначить тип сопряжению для его аргументов.
  ...
  \param forMove[in] - \ru Способ оценки для тел с поведением перемещения, иначе вражения.
...
// for testing only
//---
GCM_FUNC(void) GCM_GetProperties( const GCM_system & , MbProperties & );

//----------------------------------------------------------------------------------------
/// \ru Определить для сопряжения количество вариантов для опции выравнивания \en Determine the count of variants of alignment options for mate 
// ---
GCM_FUNC(ptrdiff_t) VolumeOfAlignOption( const ItConstraintItem & );
