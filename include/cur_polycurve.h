...
public :
 ...
 /** \ru \name Функции описания области определения кривой.
  ...
  /** \ru \name Общие функции полигональной кривой
      ...
      \{ */
  virtual size_t      GetPointsCount() const; 		///< \ru Выдать количество характерных точек. \en Get count of control points. 
  ...
  */
  ...
  virtual void        AddPoint( const MbCartPoint & pnt );               ///< \ru Добавить точку в конец массива характерных точек. \en Add point to the end of the array of control points. 
  ...
  /** \brief \ru Изменить характерные точки.
    ...
    \param[in] pntList - \ru Заданный массив характерных точек.
  ...
  */
  ...
  /** \brief \ru Вставить точку в массив характерных точек.
  ...
  */
  ...
  /** \brief \ru Перемместить параметр в область определения кривой.
    ...
    \details \ru Проверить параметр. Если он выходит за диапазон изменения параметров кривой, то 
      ...
      Оперделить индексы характерный точек, между которыми находится заданный параметр, 
      ...
      and also determine thier parametric values. \~
  ...
  */
  ...
  virtual size_t      GetParamsCount() const = 0; ///< \ru Выдать количество параметров. \en Get count of parameters. 
          ...
          ptrdiff_t   GetPointListMaxIndex() const { return pointList.MaxIndex(); }           ///< \ru Выдать максимальный индекс массива характерных точек. \en Get maximal index of array of control points. 
          void        GetPointList( SArray<MbCartPoint> & pnts ) const { pnts = pointList; }///< \ru Вернуть массив характерных точек. \en Get array of control points.  
  const   MbCartPoint & GetPointList( size_t i ) const { return pointList[i]; } ///< \ru Вернуть характерную точку с заданным индексом. \en Get control point with the given index. 
          ...
          void        SetPointList( const SArray<MbCartPoint> & pnts ); ///< \ru Заменить набор характерных точек. \en Replace the set of control points.

          ptrdiff_t   GetUppIndex() const { return uppIndex; } ///< \ru Вернуть максимальный индекс массива характерных точек. \en Get the maximal index of array of control points.  
...
//------------------------------------------------------------------------------
// \ru Заменить набор характерных точек \en Replace the set of control points 
// ---
inline void MbPolyCurve::SetPointList( const SArray<MbCartPoint> & pnts )
{
  PRECONDITION( pnts.Count() > 1 );
  Refresh();
  pointList = pnts;
}
