...
public:
  ...
  /// \ru Создать вывернутый одномерный куб. \en Create everted one-dimensional cube. 
  void   Invert     ();
  /// \ru Сократить одномерный куб на заданный коэфициент и расширить на дельта. \en Decrease one-dimensional cube by a given factor and increase by delta. 
  void   Short      ( double,         bool bis = true, double delta = LENGTH_EPSILON );
  /// \ru Сократить одномерный куб на заданный коэфициент относительно точки и расширить на дельта. \en Decrease one-dimensional cube by a given factor relative to point and increase by delta. 
  ...
  void   Equalize   (                    );
  /// \ru Является ли область вывернутой. \en Check if region is everted. 
  bool   IsEmpty    (                    ) const;
