...
*/
// ---
class MATH_CLASS MbSurfaceCurve : public MbCurve3D, public MbNestSyncItem {
protected :
...
public:
  ...
  virtual bool    ChangeCarrierBorne( MbSpaceItem &, MbSpaceItem &, const MbMatrix & matr ); // \ru Изменить носимые элементы. \en Change a carrier elements. 
  ...
  */
  ...
  /// \ru Определить, является ли кривая curve копией этой кривой. \en Determine whether the 'curve' curve is a duplicate of the current curve. 
          bool    IsSameCurvePoints( const MbSurfaceCurve * scurve, double accuracy, int b = 0 ) const;
  ...
  /// \ru Дать кривую. \en Get curve. 
          MbCurve &   SetCurve()         { cache.Reset( true ); return *curve;   }
  /// \ru Дать поверхность. \en Get surface. 
          MbSurface & SetSurface()       { cache.Reset( true ); return *surface; }
