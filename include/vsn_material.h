...
*/
//---
class VSN_CLASS Material : public Object
{
public:
  ...
  */
  ...
  Material( Texture2D* pTexture, const std::wstring& name = std::wstring() );
#endif
...
#ifdef __USE_QT__
  /// \ru Вернуть имя файла текстуры. \en Returns texture file name.
  std::wstring GetTextureFileName() const;
...
public:  
#ifdef __USE_QT__
  /// \ru Загрузка текстуры в OpenGL. \en Loads texture to OpenGL.
...
public:
  ...
  static void CreateShader( const OpenGLContext* pContext );
  /// \ru Удалить шейдер. \en Deletes shader.
  static void DeleteShader( const OpenGLContext* pContext );
  /// \ru Установить шейдер выбора из файла. \en Sets selection shader from file.
  static void SetShadersFromFile( QFile& file, QFile& fileFragment, const OpenGLContext* pContext );
  /// \ru Вернуть true, если есть шейдер выбора. \en True if selection shader exists.
  static bool IsExistShader( const OpenGLContext* pContext );
