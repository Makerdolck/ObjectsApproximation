...
public:
  ...
  */
  ...
  /// \ru Доступ по ссылке к элементу матрицы. \en Access to a matrix element by a reference. 
        double & operator() ( size_t i, size_t j )       { PRECONDITION( std_max( i, j ) < MATRIX_DIM_3D ); flag = MB_UNSET; return el[i][j]; }
  /// \ru Значение элемента матрицы. \en The value of a matrix element. 
  const double & operator() ( size_t i, size_t j ) const { PRECONDITION( std_max( i, j ) < MATRIX_DIM_3D );                  return el[i][j]; }
...
{
  MbMatrix3D staticMatrix; // \ru Для ускорения вычислений \en For speed up the calculations 
...
*/
// ---
inline bool EqualMatrices( const MbMatrix3D & m1, const MbMatrix3D & m2, double accuracy ) {
