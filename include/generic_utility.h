...
/** 
  ...
  избыточной информации для хранения указателей (может занимать в 4 раза меньше памяти).
    ...
    как std::binary_search, std::lower_bount и т.п.
  ...
  (can occupy memory in less than 4 times).
    ...
    as std::binary_search, std::lower_bount etc. \~
*/
//---
template<class KeyType, class _Pr = std::less<KeyType> > // \ru KeyType - тип элемента с опрераторами порядка "<" \en KeyType - the element type with the operators of order "<" 
class sorting_array
{
public:  
...
public:
  ...
  */
  ...
  _Myt & operator = ( const _Myt & v )
  {
    ...
    PRECONDITION( m_typeNb == v.m_typeNb );
    return *this;
  }
