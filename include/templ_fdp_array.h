...
FORVARD_DECL_TEMPLATE_TYPENAME( void  set_Farray_size( FDPArray<Type> &, size_t newSize, bool clear ) );
...
/** \brief \ru Одномерный массив указателей.
  ...
  \details \ru Одномерный массив указателей на объекты. \n
    ...
    Внимание! Деструктуирование больших массивов выполняется медленнее, чем в PArray. \n
...
public :
  ...
  void        SetSize ( size_t newSize, bool clear ); ///< \ru Указать новый размер массива. \en Set the new size of an array. 
...
private:
  ...
  TEMPLATE_FRIEND void  set_Farray_size TEMPLATE_SUFFIX ( FDPArray<Type> &, size_t newSize, bool clear );
...
// ---
template <class Type>
inline void FDPArray<Type>::SetSize( size_t newSize, bool clear ) {
  set_Farray_size( *this, newSize, clear );
}
...
// ---
template <class Type>
void set_Farray_size( FDPArray<Type> & arr, size_t newSize, bool clear )
{
