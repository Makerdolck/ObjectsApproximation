...
*/
// ---
inline long double CalcAngle0X( long double dx, long double dy )
{
...
*/
// ---
inline double CalcAngle0X( const MbCartPoint & p1, const MbCartPoint & p2 ) {
  return ::CalcAngle0X( p2.x - p1.x, p2.y - p1.y );
}
...
*/
// ---
inline double & NormalizeAngle( double & angle, double angleEpsilon = Math::AngleEps )
{
...
*/
// ---
inline void AngleToParam( double dir, bool left, double & t ) 
{
...
// ---
template <class Type>
inline double AngleBetweenVectors( const Type & v1, const Type & v2 )
{
  double dy = v1 | v2;
  double dx = v1 * v2;
...
*/
// ---
inline double CalcAngle3Points( const MbCartPoint & p1, const MbCartPoint & centre, const MbCartPoint & p3 ) {
  return ::AngleBetweenVectors( MbVector( centre, p1 ), MbVector( centre, p3 ) );
}
...
*/
// ---
inline MbCartPoint LineSegDivide( const MbCartPoint & p1, const MbCartPoint & p2 ) {
...
// ---
inline ptrdiff_t mod( ptrdiff_t i, ptrdiff_t n )
{
  ptrdiff_t m = i % n;
  return ( m >= 0 )? m : m + n;
}
...
//------------------------------------------------------------------------------
/// \ru Обмен значениями вещественных чисел. \en Swap values of the real numbers. 
// ---
...
// ---
//OV_LNX inline long CheckLRound( double x ) { return x > LONG_MAX ? LONG_MAX : x < LONG_MIN ? LONG_MIN : LRound(x); }
...
*/
// ---
inline bool InRange( double x1, double x2, double x )
{
  if ( x1 > x2 )
...
*/
// ---
inline bool IsParamOn( double tmin, double tmax, double t, double treg )
{
...
*/
// ---
inline bool InRangePlus( double x1, double x, double eps = FLT_EPSILON )
{
...
*/
//---
MATH_FUNC (void) SortSynchroArrays( SArray<double> & tt0, SArray<double> & tt2, 
...
*/
//---
MATH_FUNC (void) SortArray ( SArray<double> & tt0, double eps, bool checkCoincidentParams );
...
bool IsPlanar( const PointsVector & pnts, MbPlacement3D * place, double mEps = METRIC_EPSILON )
{
  ...
  if ( pntsCnt > 2 ) {
    ...
    for ( size_t i = 1; i < pntsCnt && noPlace; i++ ) {
      pnt_i = pnts[i];
      if ( !::EqualPoints( pnt_i, pnt0, mEps ) ) {
        for ( size_t j = 1; j < pntsCnt && noPlace; j++ ) {
          pnt_j = pnts[j];
          if ( !::EqualPoints( pnt_j, pnt0, mEps ) && !::EqualPoints( pnt_j, pnt_i, mEps ) ) {
            vx.Init( pnt0, pnt_i );
            vy.Init( pnt0, pnt_j );
            if ( !vx.Colinear( vy ) )
              noPlace = false;
...
bool IsPlanar2( const Array2<Point> & pnts, MbPlacement3D * place, double mEps = METRIC_EPSILON )
{
  ...
  if ( pCnt > 2 ) {
    ...
    PRECONDITION( !isFailed );

    if ( !isFailed )
      isPlanar = ::IsPlanar( tmpPnts, place, mEps );
  }
...
*/
// ---
MATH_FUNC (void) EigenVector( double a[SPACE_DIM][SPACE_DIM], MbVector3D & vect );
