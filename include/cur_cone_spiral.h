...
public:
  ...
  virtual void    GetPointsByEvenLengthDelta( size_t n, SArray<MbCartPoint3D> & pnts ) const;  // \ru Выдать n точек кривой с равными интервалами по длине дуги. \en Get n points of curve with equal intervals along the length of the arc. 
...
{
  //PRECONDITION( position.IsIsotropic() || ::fabs(tgAlpha) < LENGTH_EPSILON ); // \ru Проверить корректность работы \en Check the correctness of working 
