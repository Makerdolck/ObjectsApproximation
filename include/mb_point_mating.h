...
bool MbPntMatingData<Vector>::IsSame( const MbPntMatingData<Vector> & other, double accuracy ) const 
{
  ...
  if ( ::EqualVectors( *tangent, *other.tangent, accuracy ) &&
       ::EqualVectors( *tangentDer1, *other.tangentDer1, accuracy ) &&
       ::EqualVectors( *tangentDer2, *other.tangentDer2, accuracy ) &&
  ...
  if ( out.good() ) {
    ...
    for ( size_t k = 0; k < cnt && out.good(); k++ ) {
      ...
      if ( isItem && item->GetChangedPoints() != NULL ) {
        PRECONDITION( false ); // \ru KYA массив индексов должен быть пуст, т.к. он общий для всех сопряжений, им владеет заказчик операции \en KYA array of indices should be empy because it is shared between all of conjugations and owned by user of operation 
