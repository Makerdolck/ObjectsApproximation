...
public :
  ...
  template <class TypedPoint>
  bool    IntersectRect( const MbRect & rect, const SArray<TypedPoint> & points ) const
  { 
    if ( points.Count() < 4 )  //-V112
      return false;
  ...
  template <class TypedPoint>
  void    GetGabRect   (       MbRect & rect, const SArray<TypedPoint> & points ) const
  { 
    if ( points.Count() < 4 ) //-V112
      return;
...
*/
// ---
class MATH_CLASS MbGridPolygon {
public :
  SArray<uint> pIndices; ///< \ru Номера вершин в массиве точек. \en Indices of vertices in the array of points. 

public:
  /// \ru Конструктор. \en Constructor. 
  MbGridPolygon( size_t n = 0 ) : pIndices( n, 1 ) {}
  /// \ru Конструктор. \en Constructor. 
  template <class UintVector>
  explicit MbGridPolygon( const UintVector & init ) : pIndices( 0, 1 ) { pIndices += init; }
  /// \ru Деструктор. \en Destructor. 
  ~MbGridPolygon() {}
...
public:
  /// \ru Инициализация. \en Initialization. 
  template <class UintVector>
  void    Init( const UintVector & init ) { pIndices.clear(); pIndices += init; }
  ...
  KNOWN_OBJECTS_RW_REF_OPERATORS_EX( MbGridPolygon, MATH_FUNC_EX );
  KNOWN_OBJECTS_RW_PTR_OPERATORS_EX( MbGridPolygon, MATH_FUNC_EX );
  OBVIOUS_PRIVATE_COPY( MbGridPolygon )
};
