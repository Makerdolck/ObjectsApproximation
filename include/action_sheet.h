...
public:
  /// \ru Конструктор. \en Constructor. 
  MbDisplacementCalculator( double thick, double rad, double ang )
...
/** \brief \ru Буртик.
  ...
  \details \ru Буртик строится по одному или нескольким замкнутым или незамкнутым эскизам, лежащим на плоской листовой грани, а также по точкам.
           ...
           A bead from an open sketch has two tips at the start and at the end points; the tips type is specified in the operation parameters. The beed by point looks like spherical stamp.\n \~
  \param[in] solid - \ru Исходное тело.
                     \en The source solid. \~
  \param[in] sameShell - \ru Флаг удаления оболочки исходного тела.
                         \en Whether to delete the shell of the source solid. \~
...
/** \brief \ru Создать контур ребра жесткости по двум сторонам.
  ...
  \param[in] phi - \ru Угол прогиба профиля.
                   \en The contour bending angle. \~
...
public:
  ...
  /// \ru Рассчитать положение "хот"-точки : угол прогиба профиля. \en Calculate the hot point location : the contour bending angle.
  bool CalcContourPhiHotPoint( MbCartPoint3D & point, MbAxis3D & rotationAxis ) const;
