...
//  PRECONDITION( len > LENGTH_EPSILON || len < NULL_EPSILON );
  bool res = ( len >= NULL_EPSILON );
  if ( res && ::fabs(len - 1.0) > NULL_EPSILON ) {
    double one_len = 1.0 / len;
    x *= one_len;
    y *= one_len;
    PRECONDITION( ::fabs(1.0 - ::_hypot(x, y)) < LENGTH_EPSILON );
  }
  return res;
}
...
// ---
inline bool MbVector::IsDegenerate() const
{
  return ( ::fabs( x ) < Math::LengthEps ) && 
         ( ::fabs( y ) < Math::LengthEps );
}
...
// \ru Проверка на вырожденность \en Check for degeneracy 
// ---
inline bool MbDirection::IsDegenerate() const
{
  return ( ::fabs( ax ) < Math::LengthEps ) && 
         ( ::fabs( ay ) < Math::LengthEps );
}
...
//------------------------------------------------------------------------------
/// \ru Переставить два нормализованных вектора. \en Swap two normalized vectors. 
// ---
inline void Swap( MbDirection & v1, MbDirection & v2 )
{
  double tmp;
...
/// \ru Переставить два нормализованных вектора. \en Swap two normalized vectors. 
// ---
inline void Swap( MbVector & v1, MbVector & v2 )
{
  double tmp;
...
/** \brief \ru Вычисление минимального угла между двумя векторами
  ...
  \ingroup Mathematic_Base_2D
*/
MATH_FUNC (double) Angle2Vectors( const MbVector & v1, const MbVector & v2 );


//------------------------------------------------------------------------------
/// \ru Проверить векторы на равенство с заданной точностью. \en Check equality of vectors with given tolerance. 
// ---
inline bool EqualVectors( const MbVector & p1, const MbVector & p2, double eps )
{
...
//------------------------------------------------------------------------------
/// \ru Проверить векторы на равенство с заданной точностью для каждой компоненты. \en Check equality of vectors with given tolerance for each component. 
// ---
inline bool EqualVectors( const MbVector & p1, const MbVector & p2, double xEpsilon, double yEpsilon )
{
  return ( ::fabs(p1.x - p2.x) < xEpsilon && 
           ::fabs(p1.y - p2.y) < yEpsilon );
}
...
// ---
inline bool IsNull( const MbVector & vec, double eps )
{
...
// ---
inline double SetVecM( const MbVector & vF, const MbVector & vS ) {
...
//---
inline MbVector Perpendicular( MbVector vec )
{
