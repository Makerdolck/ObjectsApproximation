...
private:
  mutable use_count_type  m_countRegistrable; ///< \ru Количество использований регистрируемого объекта. \en Number of usages of the registrable object. 
...
public:
  ...
  // AVA 5/16 Внимание!!! Завязка на typeid(*this).name() не безопасна, т.к. стандарт
  // отдаёт на откуп компилятору (и даже его конкретной версии) возвращаемое 
  // функцией name() значение: http://www.cplusplus.com/reference/typeinfo/type_info/name/
...
public:
  ...
  virtual TapeBase *        Get( ClusterReference & ) const { return NULL; } // unsupported
...
public:
  ...
  virtual TapeBase *        Get ( ClusterReference & ref ) const;
...
public:
  ...
  ///< \ru Зарегистрировать указатель. \en Unregister the pointer. 
...
public:
  typedef std::auto_ptr<reader> reader_ptr;
protected:
  /// \ru Конструктор. \en Constructor. 
...
public:
  ...
  static reader_ptr   CreateReader ( std::auto_ptr<iobuf_Seq> buf, uint16 om );
...
public:
  ...
  virtual TapeBase *  ReadObjectByPosition ( ClusterReference & ) { return NULL; }
  ...
  virtual c3d::IModelTree * GetModelTree() { return NULL; } // not supported
...
{
  std::auto_ptr<c3d::IModelTree>  m_tree;
  uint32                          m_lastError;
  bool                            m_fullRead;
protected:
  /// \ru Конструктор. \en Constructor. 
...
public:
  ...
  static std::auto_ptr<reader_ex> CreateReaderEx( std::auto_ptr<iobuf_Seq> buf, uint16 om );

  /// \ru Создать читатель для буфера в памяти. \en Create reader for membuf.
  static std::auto_ptr<reader_ex> CreateMemReaderEx ( membuf & sb, uint8 om );
...
public:
  ...
  virtual void        ReadObjectCatalog();
  /// \ru Читать объект по позиции в кластере. \en Read an object by position in cluster. 
  virtual TapeBase *  ReadObjectByPosition ( ClusterReference& position );
  /// \ru Установить позицию чтения. \en Set reading position. 
  virtual bool        SetReadPosition ( ClusterReference & );

  /// \ru Получить указатель на дерево модели. \en Get pointer to the model tree. 
  virtual c3d::IModelTree * GetModelTree();
...
protected:
  ...
  virtual TapeBase *  ReadDetachedObject();
  /// \ru Зегистрировать объект. \en Register the object. 
  virtual void        RegisterObject( TapeBase * obj, uint8 regId, ClusterReference ref = ClusterReference() );
...
public:
  typedef std::auto_ptr<writer> writer_ptr;
protected:
  /// \ru Конструктор. \en Constructor. 
...
public:
  ...
  static writer_ptr   CreateWriter( std::auto_ptr<iobuf_Seq> buf, uint16 om );
...
public:
  /// \ru Создать писатель для последовательного буфера. \en Create writer for iobuf_Seq.
  static std::auto_ptr<writer_ex> CreateWriterEx( std::auto_ptr<iobuf_Seq> buf, uint16 om );

  /// \ru Создать писатель для буфера в памяти. \en Create writer for membuf.
  static std::auto_ptr<writer_ex> CreateMemWriterEx( membuf & sb, uint8  om );
...
public:
  typedef std::auto_ptr<rw> rw_ptr;
public:
  /// \ru Конструктор. \en Constructor. 
...
#ifdef __DEBUG_MEMORY_ALLOCATE_FREE_
  //----------------------------------------------------------------------------------------
  /// \ru Обьявление функций new, delete и операторов доступа. \en Declaration of functions new, delete and access operators.  \~ \ingroup Base_Tools_IO
...
inline const char * pureName( const char * name )
{
  ...
  {
#ifdef _MSC_VER    
    // \ru убираем ключевые слова "class", "struct" и т.д. в начале строки \en remove the keywords "class", "struct" and so on in the beginning of the string 
    ptrdiff_t i = strlen(name) - 1;
    for ( ; i >= 0 && name[i] != ' '; i-- );
    return ((i >= 0) && (name[i] == ' ')) ? &(name[i+1]) : name;
#else // _MSC_VER    
    // \ru убираем длину имени в начале строки \en remove the name length in the beginning of the string 
...
#ifdef _MSC_VER // \ru Код для поддержки КОМПАС \en Code for KOMPAS support 
//----------------------------------------------------------------------------------------
...
inline size_t ReadCOUNT ( reader & in, bool uint_val = true )
{
  ...
  if ( IsVersion64bit( in.MathVersion() ) )
  {
    ...
    if ( in.MathVersion() >= 0x0F001001L ) {
      ...
      PRECONDITION( _count != SYS_MAX_UINT32 ); // неоднозначность интерпретации значения // Primery_Set/Models_8/_58720. 
...
inline void HashDocumentationOut( uint16 hashValue, const char * name, bool unique )
{
  ...
  const TCHAR * outName = _T("hash_GRP.txt");
  t_ofstream out( outName, st_calls ? (ios::out|ios::app) : ios::out );
...
inline writer & operator << ( writer & out, const StrHash & strHash )
{
  ...
  //OV_x64 PRECONDITION( !(strHash.m_val > 0 && strHash.m_type == StrHash::htp_undef) );
  PRECONDITION( !(IsGood(strHash.m_val) && strHash.m_type == StrHash::htp_undef) );

  //OV_x64 out << strHash.m_val;
...
//----------------------------------------------------------------------------------------
/// \ru Записать кластер. \en Write the cluster.  \~ \ingroup Base_Tools_IO
// ---
inline void WriteCluster( writer & out, const Cluster & cl )
{
  uint16 len = cl.m_l;
  out << len;
...
//------------------------------------------------------------------------------
// \ru Записать содержимое кластера.
// \en Write the cluster's contents.
// ---
inline size_t WriteClusterBody( void * out, VERSION version, const Cluster & obj )
{
  uint8 * m = (uint8 *)out;
...
//------------------------------------------------------------------------------
// \ru Прочитать содержимое кластера.
// \en Read the cluster's contents.
// ---
inline size_t ReadClusterBody( void * in, VERSION version, Cluster & obj )
{
  uint8 * m = (uint8 *)in;
