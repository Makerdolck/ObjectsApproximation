...
protected:
  mutable VarsDerives varDers; ///< \ru Рабочие перменные. \en Working variables. \~
...
public:
  ...
  */
  virtual std::auto_ptr<BTreeNode> FixVars ( const RPArray<ItTreeVariable> & unfixedVars,
                                             PArray<ItUserFunc> & newFuncs, EquTreeResCode & code )   const = 0;

  /// \ru Дать эквивалентный узел. \en Get equivalent node. 
  virtual std::auto_ptr<BTreeNode> GetCalcEquivalent()                                                const = 0;
...
public:
  ...
  */
  ...
  virtual std::auto_ptr<BTreeNode> FixVars( const RPArray<ItTreeVariable> & /*unfixedVars*/,
                                          ...
                                          { return std::auto_ptr<BTreeNode>(Duplicate()); }

  virtual std::auto_ptr<BTreeNode> GetCalcEquivalent() const
                                          { return std::auto_ptr<BTreeNode>(Duplicate()); }
...
public :
  ...
  */
  ...
  virtual std::auto_ptr<BTreeNode> FixVars( const RPArray<ItTreeVariable> & unfixed,
                                            PArray<ItUserFunc> & newFuncs, EquTreeResCode & ) const;

  virtual std::auto_ptr<BTreeNode> GetCalcEquivalent() const { return std::auto_ptr<BTreeNode>(Duplicate()); }

  virtual void           GetString( c3d::string_t & str ) const { str += id->GetName(); }       
  virtual size_t         SizeOf() const;

  // \ru доступ к данным \en access to data 
          ItTreeVariable & GetVariable() const { PRECONDITION( id ); return *id; } ///< \ru Дать параметрический идентификатор (переменную). \en Get parametric identifier (variable). 
...
private:
  ...
  static std::auto_ptr<MbUserFunc> deriveFunc;  ///< \ru Функция для вычисления производной показательно-степенной функции \en Function for calculation of derivative of the exponential-power function  
...
public:
  ...
  */
  ...
  BTreeNode & GetFirstOperand () const { PRECONDITION( op1 ); return *op1; }
  /// \ru Второй операнд. \en The second operand. 
  BTreeNode & GetSecondOperand() const { PRECONDITION( op2 ); return *op2; }
  /** \} */  
...
public :
  ...
  // \ru выдать значение параметра для экстремума( если это возможно ). \en get value of parameter of extremum( if it is possible ).   
  virtual bool            GetCharacterPoints( std::vector<CharacterPointInfo> & ) const;
  virtual bool            GetDefRange( DefRange & range, ItTreeVariable & var, bool stopOnBreak ) const { PRECONDITION( op );
