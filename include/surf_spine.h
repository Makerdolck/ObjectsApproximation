...
const VERSION SPINE_ALG_VERSION1 = 0x0A000000L; // \ru Добавлена directionCurve и новый алгоритм вычисления direction \en Added curve 'directionCurve' and the new algorithm to calculate 'direction' 
const VERSION SPINE_ALG_VERSION2 = 0x0D000023L; // \ru Добавлено создание directionCurve в общем случае \en Added creation of 'directionCurve' in general case 


//------------------------------------------------------------------------------
/** \brief \ru Криволинейная направляющая для кинематической поверхности.
           \en Curvilinear spine for sweep surface. \~
  \details \ru Криволинейная направляющая для кинематической поверхности служит 
    для расчёта в каждой точке направляющей кривой местной система координат. \n
    ...
    Местная ось Y дополняет систему до правой системы координат.
           \en Curvilinear spine for sweep surface serves 
    for calculation of a spine curve in the local coordinate system at each point. \n
    The local axis Z is oriented in direction of the curve 'curve' tangent. \n
    The local axis Z is oriented in direction of the vector 'direction'. \n
    The local axis Y complements the system to the right coordinate system. \~
...
private:
  ...
  bool        parallel;       ///< \ru Признак параллельного переноса. \en Attribute of parallel translation. 
  bool        userDirection;  ///< \ru Пользовательский вектор или кривая ориентации матрицы преобразования. \en User vector or curve of the transformation matrix orientation. 
  MbCurve3D * directionCurve; ///< \ru Кривая векторa ориентации матрицы преобразования (может быть NULL для простой траектории). \en A curve of the transformation matrix orientation (it may be NULL for a simple trajectory). 
  double      crossSize;      ///< \ru Поперечный масштаб при построении directionCurve. \en Transverse scale in construction of 'directionCurve'. 
...
protected:
  ...
  */
  ...
  /** \brief \ru Конструктор по направляющей кривой и кривой векторa ориентации матрицы преобразования.
    ...
    \param[in] paral - \ru Признак параллельного переноса
                       \en Attribute of parallel translation. \~
  */
...
public:
  ...
  */
  ...
  const MbCurve3D  * GetDirectionCurve() const { return directionCurve; } 
  ...
  /// \ru Признак параллельного переноса. \en Attribute of parallel translation. 
  bool               IsParallel  () const   { return parallel; } 
  /// \ru Изменить признак параллельного переноса. \en Change an attribute of parallel translation. 
  void               SetParallel ( bool p ) { parallel = p; }    

  /// \ru Признак пользовательского направления. \en Attribute of user direction. 
  bool               IsUserDirection() const { return userDirection; } 
...
public:
  /** \brief \ru Конструктор по направляющей кривой.
    ...
    \param[in] version - \ru Версия операции
                         \en Version of operation \~
  */
  static  MbSpine &   Create( const MbCurve3D & c, bool parallel, bool same, VERSION version = Math::DefaultMathVersion() );
  /** \brief \ru Конструктор по направляющей кривой и вектору ориентации матрицы преобразования.
             \en Constructor by spine curve and a vector of transformation matrix orientation. \~  
    \details \ru Конструктор по направляющей кривой и вектору ориентации матрицы преобразования.\n
             \en Constructor by spine curve and a vector of transformation matrix orientation.\n \~
    ...
    \param[in] version - \ru Версия операции
                         \en Version of operation \~
  */
  static  MbSpine *   Create( const MbCurve3D & c, bool same, const MbVector3D & dir, bool parallel, VERSION version = Math::DefaultMathVersion() );
  /** \brief \ru Конструктор по направляющей кривой и кривой векторa ориентации матрицы преобразования.
             \en Constructor by spine curve and a curve of the vector of transformation matrix orientation. \~  
    \details \ru Конструктор по направляющей кривой и кривой векторa ориентации матрицы преобразования.\n
             \en Constructor by spine curve and a curve of the vector of transformation matrix orientation.\n \~  
    ...
    \param[in] paral - \ru Признак параллельного переноса
                       \en Attribute of parallel translation. \~
  */
  static  MbSpine *   Create( const MbCurve3D & sp, bool sameS, const MbCurve3D & dc, bool sameD, bool paral, VERSION version = Math::DefaultMathVersion() );
...
private:
  ...
  bool               InitDirectionCurve( bool exactOnly ); // \ru Вычисление directionCurve; использовать, если нетривиальный случай. \en Calculation of 'directionCurve'; use if the case is nontrivial. 
...
struct SpineData {
  ...
  SpineData( MbSpine & sp, const MbCartPoint3D & org, const MbVector3D & v, const MbVector3D & n, double ang )
    : spine( sp  )
    , point( org )
    , vect ( v   )
    , norm ( n   )
