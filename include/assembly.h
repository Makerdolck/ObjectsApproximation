...
private:  
  ...
  mutable ItAssemblyReactor *  m_reactor;
...
public:
  ...
  virtual void    GetProperties( MbProperties & props ); // \ru Выдать свойства объекта. \en Get properties of the object.
  virtual void    SetProperties( const MbProperties & props ); // \ru Установить свойства объекта. \en Set properties of the object.
  ...
  virtual bool    DetachSelected( RPArray<MbItem> & items, SArray<MbMatrix3D> & matrs );
...
public:
  ...
  MtGeomConstraint AddConstraint( MtMateType, MtGeomArgument, MtGeomArgument, MtParVariant = MtParVariant::undef );
  ...
  void             Import( ItAssemblyImportData & );

public:
  /** \} */
  DECLARE_PERSISTENT_CLASS_NEW_DEL( MbAssembly );
  /*
    \ru Функция устарела. Теперь сборка не имеет собственной локальной СК. Что бы наделить сборку ЛСК используете MbInstanse.
    \en This function is deprecated. Now MbAssembly does't have a proper LCS. Use MbInstanse to give the assembly its own placement.
    \ru Выдать систему координат объектов сборки. \en Get the coordinate system of assembly items.
  */
  static const MbPlacement3D & GetPlacement() { return MbPlacement3D::global; }
  ...
  , m_reactor( NULL )
{
  ...
  for ( size_t i = 0, iCount = items.size(); i < iCount; ++i ) 
  {
    ...
    {
      PRECONDITION( 0 ); // The item has already a name. It's probably means that the item is owned another assembly.
      break;
    }
  }
#endif // C3D_DEBUG
...
{
  PRECONDITION( assemblyItems.empty() && (constraintSystem == NULL) );
  ...
  for ( size_t i = 0, iCount = items.size(); i < iCount; ++i ) 
  {
    if ( MbItem * item = items[i] ) {
      if ( item->GetItemName() == UNDEFINED_SNAME ) {
        ...
        PRECONDITION( idCounter <= item->GetItemName() );
