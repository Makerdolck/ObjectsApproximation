...
//------------------------------------------------------------------------------
/** \brief \ru Структура для информации о кластере.
           \en Structure for information about the cluster. \~
  \details \ru Структура для информации о кластере (начало и длина).
    Используется для дисковых операций и в памяти. \n
           \en Structure for information about the cluster (the beginning and the length).
    It is used for the disk operations and memory operations. \n \~
  \ingroup Base_Tools_IO
*/
// ---
struct CAM
{
  uint8 * mem_m; ///< \ru Информация о кластере. \en Information about the cluster. 
  bool    own_m; ///< \ru Флаг владения данными. \en Data ownership flag. 

  /// \ru Конструктор. \en Constructor. 
  CAM( uint8 * mem, bool own )
    : mem_m( mem )
    , own_m( own )
  {}
  ...
  {
    return mem_m < o.mem_m;
  }
  /// \ru Оператор равенства. \en Operator for equality. 
  bool operator == ( const CAM & o ) const 
  {
    return mem_m == o.mem_m;
  }
...
//------------------------------------------------------------------------------
/** \brief \ru Буфер кластеров.
           \en Cluster buffer. \~
  \details \ru Буфер кластеров. \n
           \en Cluster buffer. \n \~
  \ingroup Base_Tools_IO
*/
// ---
...
{
private:
  SPArray<CAM> busy_m; ///< \ru Массив занятых кластеров. \en Array of used clusters. 
  PArray<CAM>  free_m; ///< \ru Массив свободных кластеров. \en Array of free clusters. 

public:
  /// \ru Конструктор. \en Constructor. 
  ClustersBuff();
  /// \ru Деструктор. \en Destructor. 
  virtual ~ClustersBuff();

  /// \ru Освободить неиспользуемую память. \en Free unused memory. 
  void FreeUnusedMem();
  /// \ru Получить память. \en Get the memory. 
  size_t MemGet();
  /// \ru Возвратить память. \en Return the memory. 
  bool MemReturn( uint8 * m );
};
...
private:
  ...
  // \ru Получить экземпляр буфера кластеров. \en Get an instance of class buffer. 
  ClustersBuff & StaticClustersBuff();
...
//OV_LNX   friend size_t  getMemLen   ( const Cluster& c, VERSION version ); 
  ...
  friend MATH_FUNC (size_t)  WriteClusterBody ( void *, VERSION version, const Cluster & );
  /// \ru Прочитать содержимое кластера. \en Read the cluster's contents. 
  friend MATH_FUNC (size_t)  ReadClusterBody  ( void *, VERSION version, Cluster & );
  ...
  friend MATH_FUNC (void)    WriteCluster ( writer &, const Cluster & );

  friend class  membuf;
  template<class Base>
  friend class  file_buf_base;
...
/** \brief \ru Потоковый буфер, обеспечивает только последовательную запись, без возможности удалить или перезаписать файл.
  ...
  \details \ru Потоковый буфер - базовый класс. \n
    ...
    Классы iobuf_Seq, tape, Catalog и их наследников следует рассматривать в совокупности. \n

    Терминология: \n
      - Xранилище - файл на диске или область памяти. \n
      - Каталог - каталог внутри хранилища. \n
...
private:
  uint32             stateFlag;     ///< \ru Cостояние буфера. \en The buffer state. 
                                    // todo io::state
...
//AR      int           ch   ();                   // return next char and do not advance
  ...
  virtual FileSpace *   enterFileSpace ( ClusterReference & , bool ) { return NULL; }  // не реализовано; not implemeneted
          ...
          /// If saveCurr = true, save previous position.
  virtual FileSpace *   enterFileSpace ( ClusterReference &, FileSpace *, bool ) { return NULL; }  // не реализовано; not implemeneted
...
/** \brief \ru Потоковый буфер с произвольным доступом
  ...
  При закрытии модифицированного Catalog из freed удаляются все кластеры,
  которые физически лежат после последнего занятого кластера,
  после чего содержимое freed записывается в sys.
  При записи, когда небходимо распределить новый кластер, 
  ...
  Фукнция smooth() - пригладить кластеры в буфере.
  Вызывается при закрытии буфера в случае его модификации.

  При работе с диском она удаляет все свободные кластера, 
  которые располагаются после последнего занятого, 
  после чего, пытается объединить соседние кластера,
  принадлежащие одному файлу для того, чтобы уменьшить их общее их число 
  за счет увеличения размера кластеров.
  Это дает уменьшение размера хранилища (его каталога) и ускоряет чтение. \n
  ...
  While writting, when it is necessary to allocate the new cluster, 
  ...
  Function smooth() - smooth clusters in the buffer.
  Called when closing the buffer if it was modified.

  When working with the disk, it deletes all the free clusters 
  located after the last used one. 
  After that it tries to unite neighboring clusters
  belonging to the same file to decrease thier total number 
  by increasing size of clusters.
  It allows to decrease the storage size (size of its catalog) and to speed up reading. \n
...
public:
  ...
  virtual void  smooth(); ///< \ru "Пригладить" кластеры в буфере. \en "Smooth" clusters in the buffer. 
...
//------------------------------------------------------------------------------
/// \ru Стандартный размер кластера. \en The standard size of cluster. \~ \ingroup Base_Tools_IO
//---
