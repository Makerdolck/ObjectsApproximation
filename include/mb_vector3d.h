...
// ---
inline bool MbVector3D::IsDegenerate() const
{
  return ( ::fabs( x ) < Math::region && 
           ::fabs( y ) < Math::region && 
           ::fabs( z ) < Math::region );
}
...
//  PRECONDITION( len > LENGTH_EPSILON || len < NULL_EPSILON );
  ...
  if ( res && ::fabs(len - 1.0) > NULL_EPSILON ) {
    ...
    y *= one_len;
    z *= one_len;
    PRECONDITION( ::fabs(Length() - 1.0) < LENGTH_EPSILON );
  }
  return res;
}
...
/// \ru Вычислить векторное произведение векторов. \en Calculate vector product of vectors. 
// ---
inline void SetVecM( MbVector3D & vVec, const MbVector3D & vF, const MbVector3D & vS )
{
...
/// \ru Вычислить векторное произведение векторов, умноженное на число. \en Calculate vector product of vectors multiplied by number. 
// ---
inline void SetVecM( MbVector3D & vVec, const MbVector3D & vF, const MbVector3D & vS, double mulKoef )
{
...
//------------------------------------------------------------------------------
/** \brief \ru Проверить векторы на равенство с заданной точность.
...
*/
// ---
inline bool EqualVectors( const MbVector3D & p1, const MbVector3D & p2, double eps )
{
...
*/
// ---
inline bool Nonzero( const MbVector3D & vec, double eps )
{
