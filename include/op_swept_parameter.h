...
private:
  // \ru 1) Данные о двумерных контурах на поверхности. \en 1) Data about two-dimensional contours on the surface.
  SPtr<MbSurface>                 surface;    ///< \ru Поверхность. \en The surface.
  std::vector< SPtr<MbContour> >  contours;   ///< \ru Множество двумерных контуров. \en Set of two-dimensional contours.

  // \ru 2) Трехмерные контуры. \en 2) Three-dimensional contours.
  std::vector<SPtr<MbContour3D> > contours3D; ///< \ru Множество двумерных контуров. \en Set of two-dimensional contours.

  // \ru 3) Тело. \en 3) Solid.
  SPtr<MbSolid>                   solid;      ///< \ru Тело. \en A solid.
...
public:
  ...
  */
  ...
  const std::vector<SPtr<MbContour> > &   GetContours()   const { return contours;   }
  /// \ru Выдать набор трехмерных контуров. \en Get the set of three-dimensional contours.
  const std::vector<SPtr<MbContour3D> > & GetContours3D() const { return contours3D; }
  ...
  */
  bool              SetEqual ( const MbSweptData & init );
...
public:
  ...
  {
    if ( (other.shellClosed == shellClosed) && (other.mergeFaces == mergeFaces) ) {
      if ( ::fabs(other.thickness1 - thickness1) < accuracy ) {
        if ( ::fabs(other.thickness2 - thickness2) < accuracy ) {
          if ( other.checkSelfInt == checkSelfInt )
            return true;
        }
      }
    }
...
public:
  ...
  */
  ...
  /** \brief \ru Способ привязки.
             \en Binding method. \~
    \details \ru Способ привязки: \n
             natur == true  - Тело привязано к образующей; \n
             natur == false - тело привязано к направляющей.
             \en Binding method: \n
             natur == true  - Solid is bound to generating; \n
             natur == false - solid is bound to spine. \~
  */
  bool natur;
...
public:
  ...
  virtual bool IsSame( const SweptValues & other, double accuracy ) const
  {
    const EvolutionValues * obj = dynamic_cast<const EvolutionValues *>( &other );
    if ( obj != NULL ) {
      if ( (obj->parallel == parallel) && (obj->natur == natur) ) {
        if ( obj->SweptValues::IsSame( *this, accuracy ) )
          return true;
      }
    }
  ...
  virtual bool IsSame( const SweptValues & other, double accuracy ) const
  {
    ...
    if ( obj != NULL ) {
      if ( obj->closed == closed ) {
        if ( ::EqualVectors( vector1, obj->vector1, accuracy ) && ::EqualVectors( vector2, obj->vector2, accuracy ) ) {
          if ( obj->SweptValues::IsSame( *this, accuracy ) )
            return true;
        }
      }
