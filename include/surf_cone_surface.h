...
/** \brief \ru Kоническая поверхность.
  ...
  \details \ru Коническая поверхность описывается радиусом radius, высотой height и углом конусности angle, заданными в локальной системе координат position. \n
    ...
    Значениt v=0 соответствует точке плоскости XY локальной системы координат, 
  ...
  /** \brief \ru Конструктор по трем точкам.
    ...
    \details \ru Конструктор по трем точкам.\n
      ...
      Угол С1 С2 С3 - полуугол конуса.
             \en Constructor by three points.\n
      Vector from c0 point to c1 point determines Z-axis.\n
      Vector from c0 point to c2 point determines direction of X-axis.\n
      Radius at origin of local coordinate system is equal to zero.\n
      Angle C1 C2 C3 - semi-angle of cone. \~
  */
  explicit MbConeSurface( const MbCartPoint3D & c0, const MbCartPoint3D & c1, const MbCartPoint3D & c2 );
...
public:
  ...
  /** \ru \name Функции конической поверхности
      ...
      \{ */
          ...
          /// \ru Установить высоту. \en Set height. 
          void    SetHeight( const double & h ) { height = h; tgAngleH = ( h      * ::tan(angle) ); PRECONDITION( ::fabs(height) > LENGTH_EPSILON ); } 
          /// \ru Угол. \en Angle. 
          double  GetAngle () const { return angle;  } 
          /// \ru Высота. \en Height. 
          double  GetHeight() const { return height; } 
          /// \ru Тангенс угла, умноженный на высоту. \en Tangent of the angle multiplied by height.        
...
// ---
inline void MbConeSurface::CheckParam( double & u, double & v ) const
{
  if ( (u < umin) || (u > umax) ) {
    if ( uclosed ) 
...
// ---
inline bool MbConeSurface::CheckVParam( double & v ) const
{
  bool bRes = true;
  ...
  if ( v < vmin || v > vmax ) {
    ...
    if ( ((v < vPole - EXTENT_EPSILON) && (vmin > vPole - EXTENT_EPSILON)) ||
      ...
      v = vPole;
      bRes = false; // \ru Изменили параметр v, чтобы не уйти за полюс \en V parameter was changed not to leave out of pole 
    }
  }
...
// ---
inline double MbConeSurface::GetVPole() const {  
