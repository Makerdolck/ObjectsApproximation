...
//----------------------------------------------------------------------------------------
/// \ru Вариант поведения для манипулирования недоопределенной системой сопряжений \en Variant of behavior for manipulating the underdetermined system of conjugations 
// ---
typedef enum  
{
  // \ru С произвольным поведением \en With free behavior 
    rep_FreeRotation  ///< \ru Произвольная репозиция с преимущественным вращением. \en Arbitrary reposition with predominant rotation. 
  , rep_FreeMoving    ///< \ru Произвольная репозиция с преимущественным перемещением. \en Arbitrary reposition with predominant moving. 

  // \ru С более строгим поведением \en With strict behavior 
  , rep_MovingToPoint     ///< \ru Перенос в плоскости экрана, таская курсором за точку. \en Shift in the plane of the screen dragging the cursor by the point.
  , rep_RotationAboutAxis ///< \ru Вращение вокруг неподвижной оси. \en Rotation around fixed axis. 
...
public:
  ...
  double    AsNumber() const { PRECONDITION(tag==tagReal); return numVal; }
  int       AsInteger() const { PRECONDITION(tag==tagInt); return static_cast<int>( enumVal ); }
...
public:
  ...
  */
  ...
  bool            Simple() const;
  /// \ru Выдать положение детали в виде ортонормированной ЛСК. \en Get position of an argument as a orthonormalized LCS.
...
public:
    ...
    MtPatternId     AddPattern( MtMateType, MtArgument, MtArgument );
...
//----------------------------------------------------------------------------------------
/**
  \fn MtGeomSolver::PrepareReposition( ItGeom& g_item, const MbCartPoint3D& org, const MbVector3D &axis )
  \param g_item      - \ru Компонент, деталь, на которое направлено воздействие;
                       \en Component, part on which is directed at impact; \~
  ...
  \ru Функция запускается однократно перед входом в режим вращения, который управляется 
      через команду int SolveReposition( double alpha ), где alpha - входной параметр,
      определяющий положение компонента, и заданный в радианах. Режим прекращается вызовом
      любой иной команды, кроме этих двух. Также есть специальная функция для выхода из
      этого режима  - void FinishReposition().
  \en The function runs once before running the rotation mode which is controlled 
      by command int SolveReposition( double alpha ), where "alpha" - input parameter,
      defines the position of the component and given in radians. Mode is stopped by calling
      any other command except for these two. There is the special function to exit 
      this mode - void FinishReposition(). \~
*/
