...
public: // Общие методы матриц (двумерных массивов)
   ...
   c3d::NumberPair  GetSize() const { return c3d::NumberPair( n, n ); } ///< \ru Дать размер матрицы. \en Give the size of the matrix. 
        bool        SetSize( c3d::NumberPair sz ) { size_t m = std_min( sz.first, sz.second ); return SetSize( m ); } ///< \ru Установить размер. \en Set size. 
  ...
  const double &    GetElem( size_t i, size_t j ) const     { PRECONDITION( !!parr && i < n && j < n ); return parr[i][j]; }
        /// \ru Установить элемент матрицы (i,j). \en Set an element of the matrix (i,j). 
        void        SetElem( size_t i, size_t j, double v ) { PRECONDITION( !!parr && i < n && j < n ); parr[i][j] = v; }
        /// \ru Получить элемент матрицы (i,j). \en Get an element of the matrix (i,j). 
  const double &    operator() ( size_t i, size_t j ) const { PRECONDITION( !!parr && i < n && j < n ); return parr[i][j]; }
...
public:
        /// \ru Установить элемент матрицы (i,j). \en Set an element of the matrix (i,j). 
        double &    operator() ( size_t i, size_t j )       { PRECONDITION( !!parr && i < n && j < n ); return parr[i][j]; }
        /// \ru Выдать адрес начала строки матрицы. \en Get an address of the matrix row start . 
  const double *    GetLine( size_t i ) const { PRECONDITION( !!parr && i < n ); return parr[i]; }
        /// \ru Выдать адрес начала строки матрицы. \en Get an address of the matrix row start . 
        double *    SetLine( size_t i )       { PRECONDITION( !!parr && i < n ); return parr[i]; }
        /// \ru Инициировать элемент. \en Initiate an element. 
        void        Init( size_t i, size_t j, double v ) { PRECONDITION( !!parr && i < n && j < n ); parr[i][j] = v; }
        /// \ru Установить строку. \en Set a row. 
        void        SetLine( size_t i, double * p ) { PRECONDITION( !!parr && i < n ); parr[i] = p; }
...
MbeNewtonResult TypedGaussEquation ( MatrixNN & a, Type * b, double epsilon, ProgressBarWrapper * baseProgBar = NULL )
{
  ...
  PRECONDITION( a.Lines() == a.Columns() );
