...
/** \brief \ru Дополнительные проецируемые объекты.
  ...
  \details \ru Дополнительные проецируемые объекты содержат:\n
    ...
    2. Объект безусловно владеет массивами. Eсли массивы созданы локально, то их нужно отпустить через вызов Relinquish.
...
private:
  MbProjectionsObjects( const MbProjectionsObjects & ); // \ru Нельзя (не реализовано) \en Do not use (not implemented) 
...
private: 
  void          operator =  ( const MbProjectionsObjects & ); // \ru Нельзя \en Do not use 
...
public:
  ...
  PArray<MbLump>        lumps;     ///< \ru Проецируемые объекты (владеет). \en Projected objects (owns) 
...
/** \brief \ru Построение проекций вида.
  ...
  \param[in]  prObjects - \ru Дополнительные проецируемые объекты.
                          \en Additional projected objects. \~
  \param[out] outdata   - \ru Результат.
                          \en The result. \~
  \param[in]  visMode   - \ru Настройки видимости следов проецируемых объектов.
    ...
    не относится к условным обозначениям.
...
public:
  ...
  double              m_zNear;
  // \ru Настройки видимости следов проецируемых объектов.
  //     Относится к проекциям тел, пространственным точкам, пространственным кривым,
  //     не относится к условным обозначениям.
  // \en Visibility mode of mapping.
  //    Applicable to projections of solids, spatial points, spatial curves,
  ...
  // \ru Флаг слияния подобных кривых (по умолчанию true).
  // \en Merge same curves (default true).
  bool                m_merge;
  // \ru Габариты тел до изменений.
  // \en Bounding boxes of solids before changes.
...
*/
// ---
MATH_FUNC (void) CalculateBoundsSltFast( const MbGrid & grid, 
