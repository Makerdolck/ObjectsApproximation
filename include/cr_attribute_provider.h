...
class MATH_CLASS MbNamedAttributeContainer
{
  typedef std::vector<MbAttribute *>::iterator AttrIter;
...
private:
  ...
  std::vector<MbAttribute *>  attributes; // \ru Передаваемые атрибуты. \en Attributes to pass. 

public:
  MbNamedAttributeContainer( const MbName & name );
  virtual ~MbNamedAttributeContainer();

public:
          /// \ru Записать полученные атрибуты. \en Save the received attributes. 
          void            ReceiveAttributes  ( std::vector<MbAttribute *> & attrs );
          /// \ru Скопировать атрибуты. \en Copy attributes. 
          void            DuplicateAttributes( std::vector<MbAttribute *> & attrs, MbRegDuplicate * iReg = NULL ) const;
          ...
          void            AddAttribute( MbAttribute & attr ) ;
...
public:
  ...
  const   MbName &        GetName() const { return target; }
          ...
          void            ReadAttrCont ( reader & in  );
          /// \ru Записать в поток. \en Write to stream. 
          void            WriteAttrCont( writer & out ) const;
