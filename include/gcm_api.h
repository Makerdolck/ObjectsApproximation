...
/** \brief \ru Задать линейный паттерн.
    ...
    \param[in] scale - \ru Тип масштабирования линейного паттерна (в настоящий момент можно создать
                           Линейный Паттерн только с константными расстояниями между его элементами).
                       \en Scaling type of the Linear Pattern (at the moment it's possible to create a
                           Linear Pattern only with constant distances between elements). \~ 
...
*/
// ---
GCM_FUNC(GCM_pattern) GCM_AddLinearPattern( GCM_system gSys, GCM_geom g1, GCM_geom g2 );
...
/** \brief \ru Задать угловой паттерн.
   ...
   \par      \ru Порядок удаления
    ...
    \param[in] g1 - \ru Дескриптор образца.
                    \en Descriptor of the sample. \~
    \param[in] g2 - \ru Дескриптор оси вращения углового паттерна.
                    \en Descriptor of the rotation axis of the Angular Pattern. \~
    \param[in] scale - \ru Тип масштабирования Углового Паттерна (в настоящий момент можно создать 
                           Угловой Паттерн только с константными углами между его элементами).
                       \en Scaling type of the Angular Pattern (at the moment it's possible to create an
                           Angular Pattern only with constant angles between elements). \~
...
*/
// ---
GCM_FUNC(GCM_pattern) GCM_AddAngularPattern( GCM_system gSys, GCM_geom g1, GCM_geom g2 );
...
/** \brief  \ru Сделать геометрический объект неподвижным.
  ...
  \sa GCM_FreeGeom, GCM_IsFixed
*/
//---
GCM_FUNC(void) GCM_FixGeom( GCM_system gSys, GCM_geom g );

//----------------------------------------------------------------------------------------
/** \brief  \ru Освободить объект, зафиксированный методом GCM_FixGeom.
            \en Set free geometric object fixed by GCM_FixGeom call. \~
  \sa GCM_FixGeom, GCM_IsFixed
*/
//---
GCM_FUNC(void) GCM_FreeGeom( GCM_system gSys, GCM_geom g );

//----------------------------------------------------------------------------------------
/** \brief  \ru Вернет true, если геометрический объект был зафиксирован методом GCM_FixGeom.
            \en Returns 'true' if the object is fixed by GCM_FixGeom call. \~
  \sa GCM_FreeGeom, GCM_FixGeom
*/
//---
...
//----------------------------------------------------------------------------------------
/** \brief  \ru Выдать текущее значение радиуса геометрического объекта.
            \en Get current radius value of the geometric object.
*/
//---
...
*/
GCM_FUNC(GCM_result) GCM_PrepareReposition( GCM_system gSys, GCM_geom movGeom, const MbPlacement3D & projPlane, const MbCartPoint3D & curPnt  );
...
*/
GCM_FUNC(GCM_result) GCM_SolveReposition( GCM_system gSys, GCM_geom g, const MbPlacement3D & newPos, MtRepositionMode movType );
...
/**
    ...
    \param[in] gSys   - \ru Система ограничений.
                        \en System of constraints. \~
    \param[in] curXYZ - \ru Текущее положение курсора в ГСК.
...
*/
GCM_FUNC(GCM_result) GCM_SolveReposition( GCM_system gSys, const MbCartPoint3D & curXYZ );

/**
    \brief \ru Решить систему с изменением положения компонента через одну координату.
           \en Solve the system by changing the position of the component through one coordinate. \~
    \param[in] gSys  - \ru Система ограничений.
                       \en System of constraints. \~
    ...
    \details  \ru Это функция, управляющая режимом перепозиционирования, в котором положение тела управляется
                  изменением одной координаты, например, угла вращения вокруг оси. Режим прекращается вызовом
                  #GCM_FinishReposition или любой иной командой, меняющей состояние решетеля, например,
                  #GCM_AddConstraint. Данная функция для больших систем работает значительно быстрее, чем
                  #GCM_Evaluate или #GCM_SolveReposition(GCM_system, GCM_geom, const MbPlacement3D &, MtRepositionMode).
              \en This function controls reposition mode, in which the position of the solid is controlled by
                  changing one coordinate. For example the angle of rotation around an axis. Mode is stopped by
                  calling #GCM_FinishReposition or any other command, which is changes state of the Solver, for example
                  #GCM_AddConstraint. This function is faster than the #GCM_Evaluate or
                  #GCM_SolveReposition(GCM_system, GCM_geom, const MbPlacement3D &, MtRepositionMode) for large
                  systems. \~
*/
...
*/
//---
GCM_FUNC(const GCM_diagnostic_pars &) GCM_DiagnosticPars( GCM_system gSys );
