...
enum MbeImpExpFormat {
  ief_STEP203,  ///< \ru Прикладной протокол 203 ( Проектирование с управляемой конфигурацией ).  \en Applied protocol STEP 203 (Configuration controlled design).
  ief_STEP214,  ///< \ru Прикладной протокол 214 ( Проектирование автомобилей ).                  \en Applied protocol STEP 214 (Automotive design).
  ief_STEP242,  ///< \ru Прикладной протокол 242 ( Проектирование автомобилей ).                  \en Applied protocol STEP 242 (Automotive design).  
};
...
enum MbeModelExchangeFormat {
  ...
  mxf_ACIS,       ///< \ru Интерпретировать содержимое как ACIS.            \en Read data from buffer as ACIS.
  mxf_IGES,       ///< \ru Интерпретировать содержимое как IGES.            \en Read data from buffer as IGES.
  mxf_JT,         ///< \ru Интерпретировать содержимое как JT.              \en Read data from buffer as JT.
  mxf_Parasolid,  ///< \ru Интерпретировать содержимое как Parasolid.       \en Read data from buffer as Parasolid.
  mxf_STEP,       ///< \ru Интерпретировать содержимое как STEP.            \en Read data from buffer as STEP.
  mxf_STL,        ///< \ru Интерпретировать содержимое как STL.             \en Read data from buffer as STL.
  mxf_VRML,       ///< \ru Интерпретировать содержимое как VRML.            \en Read data from buffer as VRML.
  mxf_C3D,        ///< \ru Интерпретировать содержимое как C3D.             \en Read data from buffer as C3D.
};
...
enum MbeIOPermiss {
  ...
  iop_rBRep,            ///< \ru Разрешение на чтение форм изделий в граничном представлении (только в JT).  \en Import shapes in doundary representation (JT only).
  iop_wBRep,            ///< \ru Разрешение на запись форм изделий в граничном представлении (только в JT).  \en Export shapes in doundary representation (JT only).
...
//------------------------------------------------------------------------------
/** \brief  \ru Индексы строк, пердаваемых через конвертер.
            \en Indeces of strings, transmitted through converter.\~   
\ingroup Data_Interface
*/
// ---
...
//------------------------------------------------------------------------------
/** \brief  \ru Пердставление текста при экспорте.
...
enum eMsgDetail { 
  ...
  emd_STOPNoOrBadData,                ///< \ru Файл не содержит данных или их не удалось распознать. \en File body doens not exist or incorrect.
  ...
  emd_ErrorEmptyObjectsList,          ///< \ru Списко объектов пуст.                            \en List of objects is empty.
  emd_ErrorEmptyGeomObjectsList,      ///< \ru Списко геометрических объектов пуст.             \en List of geometric objects is empty.
  ...
  emd_ErrorEmptyCurveCompositesList,  ///< \ru Список комопнент составной кривой пуст.          \en Composite curve has an empty list of composites.
  ...
  emd_ErrorEmptyOrMore2ReferencesList,///< \ru Список ссылок пуст или содрежит более 2 элементов. \en List of references is empty or contains more than 2 items.
  ...
  emd_WarningUndefinedAxis,                   ///< \ru Не определана ось.                       \en Axis is not defined.
  emd_WarningUndefinedAxisOfRevolution,       ///< \ru Не определана ось вращения.              \en Rotation axis is not defined.
  ...
  emd_WarningUndefinedTransformationOperator, ///< \ru Не определён оператор преобразования.    \en Transformatrion is not defined.
  ...
  emd_WarningDegeneratedItemWasSkipped,       ///< \ru Поигнорирован (пропущен) вырожденный объект. \en Degenerate object was missed.  
  ...
  emd_WarningSTEPPointCorrection,       ///< \ru Скорректированы коодинаты точки. \en Point location corrected. ( by BUG_73871 )
  emd_WarningSTEPEdgeCurveByVertices,   ///< \ru Кривая ребра скорректирована с учётом коодинат вершин. \en Edge curve corrected in accordance with vertices. ( by BUG_73871 )
  emd_MessageSTEPFlagChangedToF,        ///< \ru Произведена замена флага на .F.. \en Flas was set as .F. in STEP.
  emd_MessageSTEPFlagChangedToT,        ///< \ru Произведена замена флага на .T.. \en Flas was set as .T. in STEP.
  ...
  emd_WarningACISUnsupportedParametricCurveType, ///< \ru Данный подтип ACIS параметрической кривой не поддерживается. \en Parameteric curve type is not supported by SAT converter.
  ...
  emd_WarningACISCannotImportEntityId,           ///< \ru Не удалось импортироват объект с данным Id. \en Cannot import this object by SAT converter.

  emd_ErrorIGESIncorrectExternalReference,       ///< \ru Неверное имя внешней ссылки. \en Invalid external referrence in IGES.
