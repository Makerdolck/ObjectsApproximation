...
*/
// ---
MATH_FUNC (void) FacePropagate( MbeFacePropagation fp, 
...
//------------------------------------------------------------------------------
/** \brief \ru Oпределить пересечение тел в сборке.
           \en Check intersection of solids in an assembly. \~
  \details \ru Oпределить пересечение тел в сборке. \n
           \en Check intersection of solids in an assembly. \n \~
...
*/
// ---
MATH_FUNC (void) GetDistanceToSurface( const MbPlacement3D & pl, const MbCurve * curve, 
                                       const MbSurface * surf,
                                       double & lPlus, double & lMinus );
...
/** \brief \ru Создание поверхностей сечения выдавливания плоского контура.
  ...
  \param[out] surfAndDir- \ru Результат операции - поверхности и направление отсносительно direction.
...
/** \brief \ru Положить кривую в массив усекающих объектов.
  ...
  \param[in] curve - \ru Пространственная кривaя.
                     \en A space curve. \~
  \param[out] items - \ru Выходной массив пространственных объектов.
                      \en The output array of spatial objects. \~
  \warning \ru Вспомогательная функция операции TruncateShell.
...
*/
// ---
MATH_FUNC (void) AddTruncatingCurve( MbCurve3D & curve, 
                                     RPArray<MbSpaceItem> & items );
...
/** \brief \ru Проверить корректность вскрываемых граней для создания тонкостенного тела.
  ...
  \warning \ru Вспомогательная функция операции построения тонкостенного тела.
           \en An auxillary function of thin-walled solid construction operation. \~
  \ingroup Algorithms_3D
*/
// ---
...
*/
// ---
MATH_FUNC (MbSurface *) GetExtendedSurfaceCopy( MbCube & gabarit, const MbSurface & surf, VERSION version );
...
//------------------------------------------------------------------------------
/** \brief \ru Вычислить экстремали нормальной кривизны к поверхности.
           \en Calculate extremals of normal curvature of the given surface. \~
  \details \ru Вычислить экстремали нормальной кривизны к поверхности surf. \n
           \en Calculate extremals of normal curvature of surface surf. \n \~
  \param[in] surf - \ru Поверхность.
                    \en A surface. \~
  \param[out] d_max - \ru Максимальное значение нормальной кривизны.
                      \en The maximum of the normal curvature. \~
  \param[out] d_min - \ru Минимальное значение нормальной кривизны.
                      \en The minimum of the normal curvature. \~
  \param[out] maxPoints - \ru Множество точек, в которых нормальная кривизна максимальна.
                          \en The array of points for which the normal curvature is maximal. \~
  \param[out] minPoints - \ru Множество точек, в которых нормальная кривизна минимальна.
                          \en The array of points for which the normal curvature is minimal. \~
  \return \ru Возращает true, если получилось найти экстремали нормальной кривизны.
          \en Returns true if the extremals of normal curvature have been successfully found. \~
  \warning \ru Функция находится в разработке.
           \en Implementation of the function is incomplete. \~
  \ingroup Algorithms_3D
*/
// ---
MATH_FUNC (bool) CalculateExtremeCurvatures( const MbSurface           & surf, 
                                                   double              & d_max,
                                                   double              & d_min,
...
//------------------------------------------------------------------------------
/** \brief \ru Определить максимальную и минимальную нормальные кривизны для оболочки.
           \en Calculate the maximum and the minimum normal curvature for a shell. \~
  \details \ru Определить максимальную и минимальную нормальные кривизны для оболочки,
    состоящей из граней faces. \n
           \en Calculate the maximum and the minimum normal curvature for a shell
    that consists of faces 'faces'. \n \~
  ...
  \param[out] d_min - \ru Минимальное значение нормальной кривизны.
                      \en The minimum of the normal curvature. \~  
  \return \ru Возвращает true, если получилось найти экстремали нормальной кривизны.
          \en Returns true if the extremals of normal curvature have been successfully found. \~
  \warning \ru Функция находится в разработке.
           \en Implementation of the function is incomplete. \~
  \ingroup Algorithms_3D
*/
// ---
MATH_FUNC (bool) FacesMaxMinCurvature( const RPArray<MbFace> & faces, double & d_max, double & d_min );
...
/** \brief \ru Создание трёхмерной сетки по двумерной сетке.
  ...
  \details \ru Создание трёхмерной сетки по двумерной сетке. \n
    ...
    \param[out] planarGrid - \ru Tриангуляция двумерной области.
...
*/
// ---
MATH_FUNC (MbGrid *) SpaceGrid( const MbPlacement3D & place, const MbPlanarGrid & planarGrid );
...
/** \brief \ru Заменить элемент на вставку, если расстояние от начала координат до центра
  ...
  \param[in] ratioThreashhold -   \ru Пороговое значение отношения расстояния до центра габаритного куба и его диагонали, 
                                  ...
                                  \en The replacement threashhold value of the ratio of the bounding box's center to the 
                                      origin of the world to it's diagonal length. \~
  \param[in] makeCopy -           \ru Производить ли трансофрмацию на копии объекта. \~
                                  \en Is the copy of the object nust be transformed. \~
