...
/** 
  \file
  \brief \ru Одномерный массив обьектов.
...
public:
  ...
  /// \ru Установить максимальное из приращений. \en Set the maximum increment. 
          ...
          void    SetSize( size_t newSize, bool clear/*=true*/ );       ///< \ru Установить новый размер массива. \en Set the new size of an array. 
          void    Reserve( size_t n, bool addAdditionalSpace = true );  ///< \ru Зарезервировать место под столько элементов. \en Reserve space for a given number of elements. 
          ...
          void    Fill( size_t fillCount, const Type & fillData );      ///< \ru Заполнить массив значениями. \en Fill an array. 
          void    FillZero( size_t fillCount, size_t startIndex = 0 );  ///< \ru Заполнить массив байтами содержащими 0. \en Fill an array by bites consisting of 0. 
          ...
          void    SetCArray( const Type * o, size_t count );            ///< \ru Присвоить значения из c-массива. \en Assign the value from the c-array. 
...
public: 
          ...
          void    insert( InputIterator pos, const Type & e );  ///< \ru Вставить элемент перед указанным. \en Insert an element before the specified one. 
          template <class InputIterator>
          void    erase( InputIterator pos );                   ///< \ru Удалить элемент из массива по индексу. \en Delete an element from array by the index. 
          ...
          template <class InputIterator>
          void    assign ( InputIterator first, InputIterator last ); ///< \ru Присвоить массиву новое содержимое, заменив его текущее содержимое. \en Assign new contents to the array, replacing its current contents.
...
protected :
  ...
  void    AddMemory( size_t n );  ///< \ru Обеспечить место под n элементов, независимо от AutoDelta. \en Provide memory for n elements, independently from AutoDelta. 
...
{
  set_array_size( *this, i_max, true/*\ru clear - здесь неважно \en clear - it is not necessary here */ );
}
  ...
  , parr ( 0 )
{
  if ( o.upper ) {
    ...
    set_array_size( *this, o.upper, true/*\ru clear - здесь неважно \en clear - it is not necessary here */ );
  ...
  , parr ( 0 )
{
  ...
  if ( oCount ) {
    // \ru operator = копирует только o.count элементов, а нужно все до o.upper \en the operator = copies only o.count elements, but there must be copied all elements to o.upper 
    set_array_size( *this, oCount, true/*\ru clear - здесь неважно \en clear - it is not necessary here */ );
...
// ---
template <class Type>
inline void SArray<Type>::SetSize( size_t newSize, bool clear ) {
...
// ---
template <class Type>
inline void SArray<Type>::Reserve( size_t n, bool addAdditionalSpace ) {
  if ( addAdditionalSpace ) {
    size_t nn = count + n;
    if ( upper < nn )
      set_array_size( *this, nn, false/*clear*/ );
  }
  else {
    ...
    if ( upper < n )
      set_array_size( *this, n, false/*clear*/ );
    else {
      // PRECONDITION( upper <= n ); // Use SetSize!!!
    }
  }
}
...
// ---
template <class Type>
inline void SArray<Type>::AddMemory( size_t n ) {
  if ( upper - count < n )
...
template <class Type>
template <class InputIterator>
void SArray<Type>::insert( InputIterator pos, const Type & e )
{
  if ( !begin() ) {
    reserve( 1 );
    pos = begin();
  ...
  if ( begin() ) {
    const ptrdiff_t k = std::distance( (InputIterator)begin(), pos );
    if ( k >= 0 && k <= count )
      InsertInd( k, e );
  }
}
...
template <class Type>
template <class InputIterator>
void SArray<Type>::erase( InputIterator pos )
{
  if ( begin() ) {
    const ptrdiff_t k = std::distance( (InputIterator)begin(), pos );
    if ( k >= 0 && k < count )
...
// ---
template <class Type>
inline void SArray<Type>::Fill( size_t fillCount, const Type & fillData ) {
...
// ---
template <class Type>
inline void SArray<Type>::FillZero( size_t fillCount, size_t startIndex ) {
  fill_array_zero( *this, fillCount, startIndex );
}
...
inline SArray<Type> & SArray<Type>::operator = ( const SArray<Type> & o ) {
  if ( this != &o ) { // \ru при присваивании самому себе delete делать нельзя \en when it is assigned to itself operato delete should not be used 
    Flush();
    AddMemory( o.count );  // \ru обеспечить память на такое кол-во элементов \en allocate memory for the given number of elements 
    count = o.count;
    if ( count && parr )
...
inline SArray<Type> & SArray<Type>::operator = ( const std::vector<Type> & o ) {
  Flush();
  AddMemory( o.size() );  // \ru обеспечить память на такое кол-во элементов \en allocate memory for the given number of elements 
  count = o.size();
  if ( count && parr )
...
// ---
template <class Type>
inline void SArray<Type>::SetCArray( const Type * o, size_t countC )
{
  Flush();
  AddMemory( countC );  // \ru обеспечить память на такое кол-во элементов \en allocate memory for the given number of elements   
  count = countC;
  if ( count && parr )
...
inline SArray<Type> & SArray<Type>::operator += ( const SArray<Type> & o ) {
  if ( o.size() ) {
    ...
    AddMemory( addSize );  // \ru обеспечить память на такое кол-во элементов \en allocate memory for the given number of elements 
    if ( parr )
...
inline void SArray<Type>::CatchMemory() {
  if ( upper == count )
    set_array_size( *this, upper + AutoDelta(), false/*clear*/ );
}
...
template <class Type>
template <class InputIterator>
void SArray<Type>::assign( InputIterator first, InputIterator last )
{
  const ptrdiff_t newCount = std::distance( first, last );
  set_array_size( *this, newCount, true );
...
Type * add_n_to_array( SArray<Type> & to, size_t n ) {
  if ( n ) {
    to.AddMemory( n );  // \ru обеспечить память на такое кол-во элементов \en allocate memory for the given number of elements 
...
{
  arr.Flush();
  arr.AddMemory( fillCount );  // \ru обеспечить память на такое кол-во элементов \en allocate memory for the given number of elements 
  arr.count = fillCount;
  for ( size_t i = 0; i < arr.count; i++ )
  {
...
{
  arr.AddMemory( fillCount + startIndex );  // \ru обеспечить память на такое кол-во элементов \en allocate memory for the given number of elements 
...
#ifdef _MSC_VER // Linux identical int/uint and int32/uint32 
