...
public :
  ...
  virtual void    GetPointsByEvenLengthDelta( size_t n, SArray<MbCartPoint3D> & pnts ) const; // \ru Выдать n точек кривой с равными интервалами по длине дуги \en Get n points of curve equally spaced by the arc length 
  ...
  virtual void    ChangeCarrier( MbSpaceItem & item, MbSpaceItem & init ); // \ru Изменение носителя \en Change a carrier 
  ...
  virtual bool    IsSimilarToCurve( const MbCurve3D & curve ) const; // \ru Подобные ли кривые для объединения (слива) \en Whether the curves for union (joining) are similar 

          bool    IsLinear() const { return reparamType == rt_Linear; } // \ru Является ли репараметризация линейной? \en Is the repatametrization linear?
