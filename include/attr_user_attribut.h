...
#else
  typedef std::auto_ptr<membuf> UniqueMembufPtr;
#endif
protected :
...
protected:
  static MbFixAttrSet * CreateFixAttrSet( const MbUserAttribType & attrId, std::vector<MbAttribute*> & attrs );
...
private:
  ...
  std::vector<MbAttribute *> attributes; ///< \ru Атрибуты. \en Attributes. 

private:
  /// \ru Конструктор. \en Constructor. 
  MbFixAttrSet( std::vector<MbAttribute *> & attrs );
public:
  /// \ru Деструктор. \en Destructor. 
...
public:
  ...
  void                 SetAttribute ( size_t/*uint*/ index, const MbAttribute & attrib );
  /// \ru Выдать атрибуты. \en Get attributes. 
  const MbAttribute &  GetAttribute ( size_t/*uint*/ index, const MbAttribute & attrib ) const;
...
/** \brief \ru Шаблон "определения" пользовательского атрибута.
  ...
  of standard defiitions with predefined functionality. \~ 
  \ingroup Model_Attributes
 */
...
MbUserAttribute * UserAttrDefinition<AttrClass>::ReduceUserAttrib( const MbExternalAttribute & source )
{
  ...
  resAttr->InitActions( source );
  {
    ...
    size_t memLen = 0;
    {
      ...
      memBuf.closeBuff(); // до memBuf.getMemLen!!!
