...
public :
  ...
  virtual MbCurve *   Trimmed( double t1, double t2, int sense ) const;
    ...
    const MbCurve &   GetParamCurve() const { return *curve;      }
          ...
          bool        InvertNormal( MbRegTransform * ireg = NULL ); // \ru Инвертировать нормаль, если поверхность - плоскость \en Invert normal if the surface is a plane 

          bool        ChangeCarrierBorne( MbSurface & item, MbSurface & init, const MbMatrix & matr ); // \ru Изменение носителя \en Change a carrier  
...
private:
          ...
          */
          void        ChangeSurfaceToSimilar( MbSurface & newSurface, const MbMatrix & matrix, MbRegTransform * iReg );
...
// ---
bool ChangeCarrierBorne( MbSurface & item, MbSurface & init, const MbMatrix & matr, MbCurve & curve );
