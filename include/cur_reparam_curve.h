...
/** \brief \ru Репараметризованная кривая в двумерном пространстве.
  ...
  \details \ru Репараметризованная кривая служит для согласования областей определения кривых. \n
    ...
    Репараметризованная кривая имеет другию область определения и, как следствие, другую длину своих производных.
...
public :
  ...
  virtual void        IntersectHorizontal( double y, SArray<double> & cross ) const; // \ru Пересечение кривой с горизонтальной прямой \en Intersection of a curve with a horizontal line 
  ...
  virtual void        SelfIntersect( SArray<MbCrossPoint> & crossPnt ) const;        // \ru Самопересечение произвольной кривой \en Self-intersection of an arbitrary curve 
  ...
  virtual bool        IsSimilarToCurve( const MbCurve & curve ) const; // \ru Подобные ли кривые для объединения (слива) \en Whether the curves for union (joining) are similar 
  ...
  virtual void        GetPointsByEvenLengthDelta( size_t n, SArray<MbCartPoint> & pnts ) const; // \ru Выдать n точек кривой с равными интервалами по длине дуги \en Get n points of curve equally spaced by the arc length 
...
inline void MbReparamCurve::ParameterInto( double & t ) const {
  if ( reparamType == rt_Linear )
    ...
    PRECONDITION( ::fabs(a) > NULL_EPSILON );
