...
typedef enum
{
  ...
  */
  ...
  GCM_IN_PLACE        =  7, ///< \ru Точное совпадение ЛСК двух плоскостей. \en Exact coincidence of LCS of two planes.  
...
//----------------------------------------------------------------------------------------
/** \brief  \ru Коды ошибок 3d-решателя. \en Error codes of 3d-solver. \~
    \note \ru Значения этого перечисления могут быть использованы для постоянного
              хранения данных приложения и останутся неизменными в следующих версиях.
          \en Values of this enum can be used for permanent storage of app data and will 
              be kept in the future versions. \~
...
typedef enum
{
    GCM_RESULT_None = 0                     ///< \ru  Нет сообщений от решателя (нет реализованных решений, программа что-то не умеет решать ). \en No messages from the solver (not implemented solutions, the program doesn't know to solve something ). \~
  , GCM_RESULT_Ok   = 1                     ///< \ru  Успешный результат. \en The successful result. \~
  , GCM_RESULT_Satisfied = GCM_RESULT_Ok    ///< \ru  Ограничение решено. \en Constraint has been solved. \~
  ...
  /*
    Messages have been added after V9:
  */
  , GCM_RESULT_CyclicDependence             ///< \ru  Неразрешимая циклическая зависимость. \en Unsolvable cyclic dependence. \~
  , mtResCode_InvalidDependenceForOutGeom   ///< \ru  Задана входящая зависимость для выходного объекта черного ящика; \en  Given an incoming dependence of the output object of a black box; 
  , mtResCode_InvalidDependenceForOutGeoms  ///< \ru  Задана зависимость между экземплярами массива (выходными); \en  Given dependence between depended instances of the pattern (output); 
  , mtResCode_InvalidDependenceForFixGeom   ///< \ru  Задана зависимость между экз.массива и фиксированным компонентом; \en Fixed component can not be dependent geom of a pattern constraint;
  ...
  /*
    Сообщения, добавленные после выпуска V10: \en Messages have been added after V10 \~
  */
  ...
  */
  ...
  /*
    \ru Сообщения о некорректных результатах вызовов API решателя. \en Messages about incorrect results of the solver API calls. \~
  */
  , GCM_RESULT_ItsNotDrivingDimension ///< \ru Данное ограничение должно быть управляющим размерным. \en Given constraint should be a driving dimension.
...
typedef enum 
{
    GCM_NO_DEPENDENCY = 0 ///< \ru Нет односторонней зависимости. \en It means no one-directed dependecy.
...
//----------------------------------------------------------------------------------------
/// \ru Координаты точки 3D пространства. \en Сoordinates of point in three-dimensional space.
//---
...
struct GCM_diagnostic_pars
{
  ...
  GCM_diagnostic_pars() : consCount( 0 ), dof( SYS_MAX_T ) {}
};
...
/*
  ...
  { GCM_PATTERNED       GCM_geom GCM_geom GCM_geom double GCM_alignment }
  { GCM_LINEAR_PATTERN  GCM_geom GCM_geom GCM_geom GCM_scale }
  { GCM_ANGULAR_PATTERN GCM_geom GCM_geom GCM_geom GCM_scale }
...
struct GCM_c_record
{
  ...
  GCM_c_arg  args[argsN]; // \ru Аргументы ограничения 
};
...
static const GCM_g_type mgt_Point  = GCM_POINT;
static const GCM_g_type mgt_Line   = GCM_LINE; 
static const GCM_g_type mgt_Plane  = GCM_PLANE;
static const GCM_g_type mgt_Cone   = GCM_CONE;
static const GCM_g_type mgt_Sphere = GCM_SPHERE;
static const GCM_g_type mgt_Torus  = GCM_TORUS;
static const GCM_g_type mgt_Circle = GCM_CIRCLE;
static const GCM_g_type mgt_LCS    = GCM_LCS;
static const GCM_g_type mgt_Marker = GCM_MARKER;
static const GCM_g_type mgt_Spline = GCM_SPLINE;
...
static const GCM_c_type mct_FixInPlace    = GCM_IN_PLACE;
...
static const GCM_result mtResCode_Unsolvable  = GCM_RESULT_Unsolvable;
...
static const GCM_result mtResCode_InvalidArguments  = GCM_RESULT_InvalidArguments;
static const GCM_result mtResCode_MateDoubling      = GCM_RESULT_Duplicated;
...
static const GCM_result mtResCode_IncompatibleArguments    = GCM_RESULT_IncompatibleArguments;
static const GCM_result mtResCode_OverconstrainedByFixing  = GCM_RESULT_MatedFixation;
static const GCM_result mtResCode_InappropriateAlignment   = GCM_RESULT_InappropriateAlignment;
