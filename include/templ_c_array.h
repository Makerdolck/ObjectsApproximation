...
// ---
template <class Type>
inline void CcArray<Type>::SetArraySize( size_t newCount )
{
  if ( ::TestNewSize(sizeof(Type), newCount) )
  {
    ...
    #else
      delete [] parr;
      parr = newCount ? new Type [newCount] : NULL;
    #endif // USE_REALLOC_IN_ARRAYS

#if defined(C3D_DEBUG)
      count = newCount;
#ifdef __REALLOC_ARRAYS_STATISTIC_
      ::ReallocArrayStatistic( oldParr, oldSize * sizeOfType, parr, newCount * sizeof(Type), 4/*CcArray*/ );
#endif // __REALLOC_ARRAYS_STATISTIC_
#endif // C3D_DEBUG
  }
  else {
...
inline void InitCharArray( CcArray<char> &array, const char *text )
{
  if ( text ) {
    ...
    array.SetArraySize( len );
    array.Copy( text, len );
  }
  else {
    array.SetArraySize( 1 );
    array[0] = 0;
  }
}
