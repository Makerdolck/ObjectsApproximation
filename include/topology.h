...
public :
  ...
  virtual MbVertex *  DataDuplicate( MbRegDuplicate * iReg = NULL ) const; 
  virtual void        Transform( const MbMatrix3D & matr, MbRegTransform * = NULL ); // \ru Преобразовать согласно матрице. \en Transform according to the matrix. 
  ...
  virtual void        AddYourGabaritTo( MbCube & c ) const; // \ru Добавить свой габарит в присланный габарит. \en Add your own bounding box into the sent bounding box. 
...
public :
  ...
  virtual MbEdge *    DataDuplicate( MbRegDuplicate * iReg = NULL ) const; 
  virtual void        Transform( const MbMatrix3D & matr, MbRegTransform * = NULL ); // \ru Трансформация. \en Transformation. 
  virtual void        Move     ( const MbVector3D & to, MbRegTransform * = NULL );   // \ru Перемещение. \en Moving. 
  virtual void        Rotate   ( const MbAxis3D & axis, double angle, MbRegTransform * = NULL ); // \ru Вращение. \en Rotation. 
  virtual double      DistanceToPoint( const MbCartPoint3D & to ) const; // \ru Вычислить расстояние до точки. \en Calculate the distance to a point. 
  virtual void        AddYourGabaritTo( MbCube & c ) const; // \ru Добавить свой габарит в присланный габарит. \en Add your own bounding box into the sent bounding box. 
  virtual void        CalculateLocalGabarit( const MbMatrix3D & into, MbCube & cube ) const; // \ru Рассчитать габарит в локальной системы координат. \en Calculate bounding box in the local coordinate system. 
  ...
  /// \ru Проверить ребро на совпадение с ребром edge. \en Check whether an edge coincides with another edge. 
  virtual bool        IsSame( const MbEdge & edge, double accuracy ) const; 
  ...
  /// \ru Установить направление по отношению к кривой. \en Set the direction relative to the curve. 
          ...
          MbVertex  & GetBegVertex() const { return *begVertex; } 
          /// \ru Выдать вершину-конец. \en Get the end vertex. 
          MbVertex  & GetEndVertex() const { return *endVertex; } 
          /// \ru Выдать вершину по номеру (0 - вершина-начало, 1 - вершина-конец). \en Get vertex by number (0 - start vertex, 1 - end vertex). 
          MbVertex  & GetVertex( size_t i ) const { return i ? *endVertex : *begVertex; }  
          /// \ru Выдать вершину-начало. \en Get the start vertex. 
          MbVertex  * GetBegVertexPointer() const { return begVertex; } 
          /// \ru Выдать вершину-конец. \en Get the end vertex. 
          MbVertex  * GetEndVertexPointer() const { return endVertex; } 
          ...
          MbVertex  & GetTMinVertex() const; 
          /// \ru Вылать вершину, соответствующую конечной точке кривой. \en Get a vertex corresponding to the end point of a curve. 
          MbVertex  & GetTMaxVertex() const; 
          ...
          MbVertex  * IsConnectedWith( const MbEdge & edge ) const;
          ...
          MbCurve *   GetProjection( const MbPlacement3D & place, VERSION version ) const;
...
public :
  ...
  virtual bool        IsSame( const MbEdge & edge, double accuracy ) const; // \ru Являются ли объекты равными? \en Determine whether objects are equal. 
  ...
  const   MbCurve3D * GetSpaceCurve() const; 
          ...
          */
          ...
          bool        GetTraverses( const MbFace * face, bool plus, double t, 
          ...
          bool        GetOutTraverse( const MbFace * face, bool plus, double t, MbCartPoint3D & q0, MbVector3D & outv,
          ...
          */
          ...
          /**\ru Скопировать из копии готовые метрические оценки, которые в оригинале не были расчитаны.
          ...
          */
          ...
          /** \brief \ru Разбить ребро по параметрам его кривой на несколько его частей.
            ...
            \details \ru . Если beginSafe == true  - ребро сохранит начальный участок, 
              ...
              Параметр 'surface' необходим только для толерантной кривой пересечения.
              Контейнер 'edges' содержит отрезанные части. Оотрезанные части встраиваются в циклы смежных граней.
                     \en . If beginSafe == true then the edge saves its starting piece, 
              If beginSafe == false then the edge saves its ending piece. 
            ...
            \return \ru Возвращает true, если ребро было порезано.
                    \en Returns true, if the edge was cutted. \~
          */
          ...
          /** \brief \ru Разбить ребро по точкам изменения выпуклости-вогнутости.
            ...
            \details \ru Разбить ребро по точкам изменения выпуклости-вогнутости и сложить отрезанные части в контейнер 'edges'.
                         Оотрезанные части встраиваются в циклы смежных граней.
                     \en Split the edge by points where the convexity changes. 
                         The container 'edges' contains cuted parts of edge. The cutув parts will be embedded in the loops of adjacent faces. \~
            \param[out] edges - \ru Отрезанные части ребра.
                                \en The container of cut parts. \~
            \return \ru Возвращает true, если ребро было порезано.
            \en Returns true, if the edge was cutted. \~
          */
          ...
          /** \brief \ru Продолжить ребро.
            ...
            \param[in/out] t - \ru Параметра на проложении кривой ребра.
                                \en Parameter outside of the intersection curve. \~
            \param[in] begin - \ru Начало (true) или конец (false) ребра продолжить.
                                \en The edge shold be prolonged by the beginning (true) or by the ending (false). \~
...
public :
  ...
  const   MbCurve3D & GetCurve() const { return curveEdge->GetCurve(); } 
          ...
          /// \ru Выдать вершину-начало. \en Get the start vertex. 
          MbVertex  & GetBegVertex() const; 
          /// \ru Выдать вершину-конец. \en Get the end vertex. 
          MbVertex  & GetEndVertex() const; 
          /// \ru Установить вершину-начало. \en Set the start vertex. 
          void        SetBegVertex( MbVertex & ver ); 
          /// \ru Установить вершину-конец. \en Set the end vertex. 
          void        SetEndVertex( MbVertex & ver ); 
...
public :
  ...
  MbLoop (); 
  /// \ru Конструктор по ребру. \en Constructor by data. 
  MbLoop ( MbOrientedEdge & initEdge ); 
  /// \ru Конструктор по массиву ребер. \en Constructor by array of points. 
...
public :
  ...
  virtual MbeTopologyType  IsA() const; // \ru Тип элемента. \en A type of element. 
          ...
          size_t      GetEdgesCount() const { return edgeList.Count(); } 
          ...
          MbOrientedEdge * GetOrientedEdge( size_t index ) const { return (index < edgeList.Count()) ? edgeList[index] : NULL; }
...
public:
  ...
  MbFace( const MbSurface & initSurf, bool sense );
...
public:
  ...
  virtual MbFace *    DataDuplicate( MbRegDuplicate * ireg = NULL ) const; 
  virtual void        Transform( const MbMatrix3D & matr, MbRegTransform * = NULL ); // \ru Трансформация. \en Transformation. 
  virtual void        Move     ( const MbVector3D & to, MbRegTransform * = NULL );   // \ru Перемещение. \en Moving. 
  virtual void        Rotate   ( const MbAxis3D & axis, double angle, MbRegTransform * = NULL ); // \ru Вращение. \en Rotation. 
  virtual double      DistanceToPoint( const MbCartPoint3D & to ) const; // \ru Вычислить расстояние до точки. \en Calculate the distance to a point. 
  ...
  virtual bool        GetPlacement( MbPlacement3D * ) const;
          ...
          /// \ru Соирентировать ось Х плейсмента вдоль линии его пересечения с поверхностью грани, ось Y - с нормалью. \en Orient the axis X of a placement along the line of its intersection with a surface of a face,  the axis Y - with a normal. 
          ...
          MbLoop    * GetLoop( size_t index ) const { size_t count = loops.Count(); return count ? loops[index % count] : NULL; }
          ...
          */
          ...
          /// \ru Совпадает ли грань с заданной гранью? \en Does a face coincide with the given face? 
          bool        IsSame( const MbFace & face, double accuracy ) const; 
          /// \ru Построить полигональную копию грани и заполнить ею упрощённый объект (сетку) mesh. \en Construct a polygonal copy of an edge and fill a simplified object (a mesh) by it. 
          ...
          bool        GetNearestEdge( const MbCartPoint & pOnFace, c3d::NumberPair & edgeLoc, double & tEdgeCurve, bool & orientation, double & distance ) const;
          ...
          bool        GetRectIntersectingEdges( const MbRect & rect, std::vector<c3d::NumberPair> & edgeLocs, double eps ) const;
          ...
          */
          ...
          /** \brief \ru Найти параметрическое расстояние до ближайшей границы.
            ...
            \param[out] loopNumber - \ru Идекс ближайшего цикла.
                                     \en Index of nearest loop. \~
            \param[out] loopNumber - \ru Идекс ближайшего ребра в цикле.
          ...
          */
          ...
          /// \ru Дать параметры поверхности по параметрам грани (0 <= faceU <= 1, 0 <= faceV <= 1). \en Get surface parameters by the face parameters (0 <= faceU <= 1, 0 <= faceV <= 1). 
          void        GetSurfaceParam( const double   faceU,
                                       const double   faceV,
                                             double & surfaceU,
                                             double & surfaceV ) const;
          /// \ru Дать точку грани для её параметров (0 <= u <= 1, 0 <= v <= 1). \en Get a point on a face by its parameters (0 <= u <= 1, 0 <= v <= 1). 
          void        Point( double u, double v, MbCartPoint3D & p ) const;  
          ...
          bool        GetTopologyState( ptrdiff_t & cntAdjacentFaces, RPArray<MbFace> *& adjacentFaces,
