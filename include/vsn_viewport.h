...
public:
  ...
  /// \ru Загрузить изображение фона viewport из QImage. \en Loads viewport background image from QImage. \~
...
#endif
  /// \ru Oчистить фон специальным цветом. \en Clears background with special colour. \~
...
public:
  ...
  std::vector<MbCartPoint3D> UnProject( const std::vector<int>& vectorX, GLenum buffer = GL_FRONT ) const;
...
public:
  /// \ru Вернуть идентификатор ближайшего объекта и вызвать updateOpenGL. \en Returns identifier of the nearest object and calls updateOpenGL. \~
  uint PickRender( int x, int y, GLenum buffer = GL_BACK );

  /// \ru Вернуть идентификатор объекта без вызова updateOpenGL. \en Returns object identifier without calling updateOpenGL. \~
  uint Pick( int x, int y, GLenum buffer = GL_BACK );

  /// \ru Вернуть идентификатор внутренней геометрии viewObject. \en Returns identifier of viewObject internal geometry. \~
  uint PickGeometry( ViewObject3D* viewObject, int x, int y, GLenum buffer = GL_BACK );

  /// \ru Вернуть идентификатор геометрии и примитива геометрии viewObject. \en Returns identifier of viewObject geometry and geometry primitive. \~
  void PickGeometryAndPrimitive( ViewObject3D* pViewObject, int x, int y, int& id1, uint& id2, GLenum buffer = GL_BACK );

  /// \ru Вернуть идентификаторы объектов внутри указанного прямоугольника в экранных координатах. \en Returns object identifiers inside the specified rectangle in screen coordinates. \~
