...
/** \brief \ru Эквидистантная продолженная кривая.
  ...
  \details \ru Эквидистантная продолженная кривая строится смещением точек базовой кривой вдоль нормали к ней. \n
    Параметр distance задаёт смещение точек базовой криаой.
	  Радиус-вектор кривой в методе PointOn(double&t,MbCartPoint3D&r) описывается векторной функцией\n
    r(t) = basisCurve(t) + (distance normal(t)), где normal(t) - нормаль базовой кривой \n
    ...
    The "distance" parameter sets shift of base curve points.
	  Radius-vector of the curve in the method PointOn(double&t,MbCartPoint3D&r) is described by the function\n
    r(t) = basisCurve(t) + (distance normal(t)), where normal(t) - normal of base curve \n
...
protected :
  ...
  double         distance;     ///< \ru Смещение от базовой кривой по нормали к ней. \en The offset from the base curve along normal to its. 
  double         dmin;         ///< \ru Приращение std_min параметра базовой кривой. \en The increment std_min of base curve parameter. 
  double         dmax;         ///< \ru Приращение std_max параметра базовой кривой. \en The increment std_max of base curve parameter. 
  ...
  MbMatrix       transform;    ///< \ru Матрица преобразования. \en A transformation matrix. 
...
public :
  MbOffsetCurve( const MbCurve &, double dist, double t1, double t2, bool same );      
  MbOffsetCurve( const MbCurve &, double dist, double t1, double t2, const MbMatrix & matr, bool same );
  MbOffsetCurve( const MbCurve &, double dist, bool same );
  MbOffsetCurve( const MbCurve &, double dist, const MbMatrix & matr, bool same );
protected :
  MbOffsetCurve( const MbOffsetCurve & );
public :
...
public :
  ...
  virtual bool        IsSimilarToCurve( const MbCurve & curve ) const; // \ru Подобные ли кривые для объединения (слива) \en Whether the curves are similar for merge (joining) 
  ...
  virtual void        SetBasisPoints( const MbControlData & ); // \ru Изменить объект по контрольным точкам. \en Change the object by control points.
          ...
          void        SetDistance( double d ); // \ru Установить значение смещения \en Set the offset value 
  const   MbRect &    GetGabarit() const { if ( rect.IsEmpty() ) CalculateGabarit( rect ); return rect; }
          void        SetDirtyGabarit() const { rect.SetEmpty(); }
  const   double &    GetDmin() const { return dmin; } // \ru Дать расширение начала \en Get extension of start 
  const   double &    GetDmax() const { return dmax; } // \ru Дать расширение конца \en Get extension of end 
          void        SetDmin( double d ) { dmin = d; } // \ru Установить расширение начала \en Set extension of start 
          void        SetDmax( double d ) { dmax = d; } // \ru Установить расширение конца \en Set extension of end 

          double      GetBegExtend() const { return dmin; } // \ru Дать расширение начала \en Get extension of start 
          double      GetEndExtend() const { return dmax; } // \ru Дать расширение конца \en Get extension of end 
