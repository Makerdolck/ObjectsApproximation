...
/** 
  ...
  \details \ru Программный  интерфейс  геометрического  решателя представляет
            ...
            6)  Другая группа вызовов отвечает за способы управления недоопределенной 
                  системой ограничений. Вызовы #GCE_PrepareMovingOfPoint, #GCE_MovePoint
           ...
           \en A program interface of geometric solver represents
            ...
            6) Other group of calls responses for the ways of control of underdetermined 
                system of constraints. Calls of  #GCE_PrepareMovingOfPoint, #GCE_MovePoint
...
/** \brief \ru Получить текущие координаты вектора.
  ...
  \param[in] vType  - \ru Идентификатор вектора, принадлежащего объекту (в настоящий момент равен или GCE_DIRECTION, или GCE_ORIENTATION).
                      ...
                      \en Identifier of vector belonging to the object (currently it equals DIRECTION or GCE_ORIENTATION).
...
/** \brief \ru Заморозить геометрический объект.
  ...
  \note \ru В CAD-системе КОМПАС-3D эта команда применяется для фиксации проекционной
        геометрии в ассоциативных чертежах или в эскизах с проекциями трехмерных
        объектов.
        \en In the CAD system KOMPAS-3D this command is used only for fixation of projection
        geometry in associative drawings or sketches with projections of three-dimensional
        objects. \~
*/
//---
...
*/
//---
GCE_FUNC(constraint_item) GCE_AddUnaryConstraint( GCE_system gSys
...
*/
// ---
GCE_FUNC(constraint_item) GCE_AddCoincidence( GCE_system gSys, geom_item p[2] );
...
*/
//---
GCE_FUNC(constraint_item) GCE_AddPointByMetricPercent( GCE_system gSys, geom_item curve
                                                     , geom_item pnt[3], double k );
...
*/
//---
GCE_FUNC(constraint_item) GCE_AddPointOnParEllipse( GCE_system gSys
                                                  , geom_item pnt
                                                  , geom_item ellipse
                                                  , double t );
...
/** \brief \ru Задать ограничение "Средняя точка".
  ...
  \return \ru Дескриптор нового ограничения.
          \en Descriptor of a new constraint. \~
  \details \ru Для заданных трех точек плоскости, задать отношение, связывающее тройку точек, 
  так, что третья точка лежит на середине отрезка между pnt[0] и pnt[1].
           \en For the given three points on a plane set the relation connecting the point triplet 
...
*/
//---
GCE_FUNC(GCE_result) GCE_PrepareDraggingPoint( GCE_system gSys, GCE_dragging_point drgPnt, GCE_point curXY );
...
/** \brief \ru Инициализировать режим драггинга контрольной точки множества объектов.
  ...
  дескрипторами, но имеющих одинаковые координаты.
             \en This method is intended for the group dragging
  of few objects with the "common" hot-point. A "common" hot-point is not necessarily 
  the only point (with the only descriptor), but a set of points with different
  descriptors but with equal coordinates. \~
...
/** \brief \ru Переместить точку драггинга.
  ...
  a new state of geometric objects satisfying to all the constraints 
  created before (a new solution). In this case it not required to call #GCE_Evaluate for the conversion 
...
/** \brief \ru Устаревшая функция. Вызов будет удален в одной из следующих версий.
    ...
    \attention \ru Время жизни экземпляра класса crv опирается на счетчик ссылок, т.е. 
              решатель его увеличивает при добавлении кривой и декрементирует 
              при удалении кривой из решателя.
             \en The lifetime of the instance of the class 'crv' is based on the reference counter, i.e. 
              the solver increases it when adding a curve and decreases 
              when deleting a curve from the solver. \~
*/
//---
