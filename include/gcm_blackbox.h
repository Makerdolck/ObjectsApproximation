...
/** \brief  \ru Чёрный ящик.
            \en Blackbox. \~
  \details  \ru Черный ящик реализует закон позиционирования геометрических 
                объектов, зависящих от позиции других объектов. Интерфейс 
                #ItGCBlackbox реализуется обычно на стороне клиентского приложения 
                и передается геометрическому решателю для исполнения через метод 
                #MtGeomSolver::AddBlackbox. В Cad-системе KOMPAS-3D черный ящик 
                применяется для моделирования массивов тел (паттерны), которые 
                родились путем многократного копирования оригинала с использованием 
                некоторого закона размножения экземпляров и позиционирования. 
                Черный ящик может применяться не только для организации паттернов, 
                но и для задания на стороне клиентского приложения различных 
                однонаправленных зависимостей, с определенным законом позиционирования 
                тел. Объекты, которые рассматриваются, как входящие для черного 
                ящика, называются независимыми. Объекты, которые рассматриваются, 
                как исходящие для черного ящика, называются зависимыми.
            \en Blackbox implements a law of positioning of geometric objects which are 
                dependent on positions of other objects. Interface #ItGCBlackbox is usually 
                implemented on the side of client application and it is transferred from 
                client application to the solver by method #MtGeomSolver::AddBlackbox. \~
