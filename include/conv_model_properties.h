...
class             ConvModelAssembly;
class             ConvModelDetail;
...
*/
typedef std::vector<SPtr<MaAnnotationItem> > vector_of_annotation;
...
//------------------------------------------------------------------------------
/** \brief \ru Типы линий, пердаваемых через конвертер.
...
//------------------------------------------------------------------------------
/** \brief \ru Отображение точек, пердаваемых через конвертер.
...
public:
  ...
  */
  virtual SPtr<ItModelDetail>   CreateDetail( const MbPlacement3D &place, const std::vector< SPtr<MbItem> > & componentItems, const c3d::string_t& fileName ) = 0;
  ...
  */
  ...
  \return \ru Экземпляр детали, если операция прошла успешно, NULL в противном случае.
          \en Instance of the part if the operation succeeded, NULL - otherwise. \~
  */
  virtual SPtr<ItModelDetail>   GetInstanceDetail( ) = 0;
...
public:
  ...
  */
  ...
  /// \ru Выдать ЛСК, общую для элементов комопнента. \en Get the placement, which all the items of the component use for transformation.
...
*/
// ---
class ItModelDetail : public ItModelInstanceProperties
{
public:
...
public:
  ...
  /** \brief \ru Прообраз новой интерфейсной функции - задать модель ЛСК, относительно которой позиционируется модель.
             \en Prototype of a new interface function - get the placemet the model is defined in. \~
  */
  ...
  */
  ...
  \return \ru Экземпляр детали, если операция прошла успешно, NULL в противном случае.
          \en Instance of the part if the operation succeeded, NULL - otherwise. \~
  */
  virtual SPtr<ItModelDetail>   CreateDetail( const std::vector< SPtr<MbItem> > & componentItems, const c3d::string_t& fileName ) = 0;
...
//------------------------------------------------------------------------------
/** \brief \ru Реализация интерфейса документа модели сборки или детали.
           \en Implementation of the interface of a document of an assembly model or a part model. \~
\details \ru Используется в системе тестирования трансляции.
         \en It is used in the system of translation testing. \~
\ingroup Exchange_Interface
*/
// ---
class CONV_CLASS ConvModelDocument: public ItModelDocument
{
private:
  ConvModelAssembly * assembly;   ///< \ru Сборки для экспорта и импорта. \en Assemblies for export or import. \~
  ConvModelDetail   * detail;     ///< \ru Детали для экспорта и импорта. \en Details for export or import. \~
  MbItem            * content;     ///< \ru Геометрическая модель для экспорта и импорта. \en Geometric model for export or import. \~
  map_of_visual_items annotationItems; ///< \ru Аннатационные данные модели для экспорта и импорта. \en Annotation data of the model for export or import. \~
...
public:
  ...
  */
  ...
  //virtual MbPlacement3D     GetOriginLocation() const; // Получить ЛСК, относительно которой позиционирована модель

  virtual void              SetContent( MbItem* /*content*/ ); // Задать модель в виде дерева компонент

  virtual MbItem *          GetContent() ; // Получить модель в виде дерева компонент

          void              SetSolids( const IFC_Array<MbSolid> & solids );

  // \ru Реализация интерфейса ItModelDocument \en Implementation of interface ItModelDocument 

  /// \ru Это сборка? \en Is it an assembly? 
  virtual bool              IsAssembly() const; 
  /// \ru Это ни сборка, ни деталь? \en Is it neither an assembly nor a part? 
  virtual bool              IsEmpty() const;
  // \ru Импорт. Создаёт документ с новой сборкой. \en Import. Creates a document with a new assembly. 
  virtual SPtr<ItModelAssembly> CreateAssembly( const std::vector< SPtr<MbItem> > & componentItems, const c3d::string_t& fileName );
  // \ru Импорт. Создаёт документ с новой деталью. \en Import. Creates a document with a new part. 
  virtual SPtr<ItModelDetail>   CreateDetail( const std::vector< SPtr<MbItem> > & componentItems, const c3d::string_t& fileName );
  // \ru Экспорт. Выдать интерфейс сборки или NULL. \en Export. Get interface of an assembly or NULL. 
  virtual SPtr<ItModelAssembly> GetInstanceAssembly( );
  /// \ru Экспорт. Выдать интерфейс детали или NULL. \en Export. Get interface of the model or NULL. 
  virtual SPtr<ItModelDetail>   GetInstanceDetail( );

  /// \ru Получить элементы аннотации. \en Get annotation elements. 
  virtual map_of_visual_items GetAnnotationItems( eTextForm ) const; 
  /// \ru Задать элементы аннотации. \en Set annotation elements. 
  virtual void              SetAnnotationItems( const map_of_visual_items& ); 

  /// \ru Завершить импорт, сохранить документ. \en Complete the import and save the document. 
  virtual bool              FinishImport( IProgressIndicator * indicator  );
  /// \ru Открыть документ. \en Open a document. 
  virtual void              OpenDocument();

  // \ru САА K13   // Реализация интерфейса IfSomething \en САА K13   // Implementation of interface IfSomething 
  // \ru САА K13   I_DECLARE_SOMETHING; \en САА K13   I_DECLARE_SOMETHING; 

private:
  ConvModelDocument( const ConvModelDocument & ); // \ru не реализовано \en not implemented 
  void operator= ( const ConvModelDocument & );   // \ru не реализовано \en not implemented 
};
...
*/
// ---
class CONV_CLASS RegularModelDocument: public ItModelDocument {

  SPtr<ItModelDetail> detail; ///< \ru Представление в виде детали. \en Representation as detail.
...
public:
  ...
  virtual ~RegularModelDocument(); ///< \ru Деструктор. \en Descructor.
  ...
  void    RegisterAnnotation( SPtr<MbItem> component, const vector_of_annotation& annotation );
};
...
public:  
  ...
  MbeImpExpFormat   exportSTEPFormat; ///< \ru Протокол STEP для экспорта. \en STEP applied protocol for export.
  ...
  bool              replaceLocationsToRight; ///< \ru Следует ли принудительно преобразовывать ЛСК объектов к правым (для форматов, допускующих левые). \en Force replacement of locations to right ones.
  bool              enableAutostitch; ///< \ru Сшивать ли поверхности атвоматически. \en Automatically stitch surfaces into shells.
  ...
  MbStepData        tesseleationStepData; ///< \ru Параметры триангуляции при экспорте в STL и VRML. \en Tesselation parameters for export into STL and VRML.
  MbStepData        LOD0StepData; ///< \ru Параметры триангуляции при экспорте в JT. \en Tesselation parameters for export into JT.
  ...
  /// \ru Сведения о сообщени конверетра. \en Converter message data.
  struct LogRecord {
...
public:
  ...
  /// \ru Задать протокол STEP для экспорта. \en Set STEP application protocol for export 
