...
enum MbeSpaceType {
  ...
  st_EvolutionSurface         = 311,  ///< \ru Кинематическая поверхность. \en Sweep with guide curve surface.
  st_ExactionSurface          = 312,  ///< \ru Кинематическая поверхность с поворотными торцами. \en Sweep with guide curve surface with rotating ends.
  st_ExpansionSurface         = 313,  ///< \ru Плоскопараллельная поверхность. \en Plane-parallel surfaces.
  ...
  st_NurbsSurface             = 325,  ///< \ru NURВS поверхность, определяемая кривыми. \en NURVS surface defined by curves.
  ...
  st_CompositeFilletSurface   = 338,  ///< \ru Составная поверхность скругления. \en Composite fillet surface.
...
public :
  ...
  */
  ...
  /** \brief \ru Создать полигональную копию.
             \en A polygonal copy. \~
    \details \ru Создать полигональную копию данного объекта, представленную полигонами.
                 Полигональный объект будет содержать только полигоны MbPolygon3D.
             \en A polygonal copy of the object that is represented by polygons.
                 Polygonal object will contain only polygons MbPolygon3D. \~
    \param[in]      sag -  \ru Максимальное отклонение полигонального объекта от оригинала по прогибу.
                           \en The maximum deviation from the original object polygonal by deflection. \~
    \param[in, out] mesh - \ru Принимающий полигональный объект.
                           \en Adopting polygonal object.
  */
  virtual void    CalculateWire( const MbStepData & stepData, MbMesh & mesh ) const = 0;
          void    CalculateWire( double sag, MbMesh & mesh ) const; // The method deprecated. It will be removed at 2018. Use CalculateWire( MbStepData(ist_SpaceStep,sag), mesh ); \~
