...
public :
  ...
  MbeIntersectionType     form; ///< \ru Тип точки перечечения (ipt_Simple - нормальное пересечение, ipt_Tangent - касание). \en Type of intersection point (ipt_Simple - simple intersection, ipt_Tangent - tangent). 
public:
  /// \ru Конструктор по умолчанию. \en Default constructor. 
  MbCrossPoint()
    : p()
