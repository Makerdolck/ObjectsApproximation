...
public:
  ...
  */
  MbFilletSurface( MbSurfaceCurve & curv1, MbSurfaceCurve & curv2,
                   double d1, double d2, MbeSmoothForm fm, double cn, bool ev ); // \ru Конструктор обычной поверхности скругления \en Constructor of ordinary fillet surface  
  ...
  */
  MbFilletSurface( MbSurfaceCurve & curv1, MbSurfaceCurve & curv2,
                   double d1, double d2, MbeSmoothForm fm, double cn, bool byFirst, bool ev ); // \ru Конструктор поверхности с сохранением кромки \en Constructor of surface with preservation of fillet 
  ...
  /** \brief \ru Конструктор поверхности скругления.
    ...
    \param[in] curv - \ru Кривая пересеченния касательных к поверхностям
  ...
  */
  MbFilletSurface( MbSurface & surf1, MbCurve & curv1,
                   ...
                   MbCurve3D &curv, double d1, double d2, MbeSmoothForm fm, double cn, bool ev );  // \ru Конструктор обычной поверхности скругления. \en Constructor of ordinary fillet surface.  
...
public:
  ...
  virtual bool    ChangeCarrierBorne( MbSurface & item, MbSurface & init, const MbMatrix & matr ); // \ru Изменение носимых элементов. \en Change a carrier elements. 
  ...
  /** \ru \name Функции поверхности скругления
    ...
    const MbCurve3D & GetCurve0() const { return *curve0; }
          ...
          */
          bool    IsConic() const { return ( ::fabs(conic - _ARC_) >= EPSILON ); } // \ru Коническое сечение общего вида \en General conic section 
...
//------------------------------------------------------------------------------
// \ru Создать поверхность \en Create surface 
// ---
