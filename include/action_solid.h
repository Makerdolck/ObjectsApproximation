...
/** 
  ...
  \brief \ru Функции создания тел, операции с телами.
         \en Funcions for creation of solids, operations on solids. \~
...
/** \brief \ru Создать элементарное тело.
  ...
  \param[in] points - \ru Набор точек. \n
             ...
             points[2] определяет радиус цилиндра, радиус конуса, радиус сферы, 
             ...
             points[2] determines the radius of a cylinder or a cone, radius of a sphere, 
...
/** \brief \ru Создать кинематическое тело и выполнить булеву операцию.
  ...
  \param[in]  params        - \ru Параметры кинематической операции.
                              \en Parameters of the sweepping operation. \~
  \param[in]  oType         - \ru Тип булевой операции.
                              \en A boolean operation type. \~
...
/** \brief \ru Выполнить булеву операцию.
  ...
  \details \ru Функция выполняет указанную булеву операцию над двумя телами.\n
           \en The function performs the specified Boolean operation on two solids.\n \~
...
/** \brief \ru Создать тело путем булевой операции.
  ...
  \details \ru Создать тело путем булевой операции типа oType для тел solid1 и solid2. 
    ...
    bo_Intersect  - пересечение, \n
    bo_Difference - вычитание. \n
    Функция работает только с замкнутыми телами. 
    ...
    bo_Intersect  - intersection, \n
    bo_Difference - subtraction. \n
    The function accepts only closed solids. 
...
/** \brief \ru Разрезать тело поверхностью.
  ...
  \param[in] sameShell - \ru Режим копирования исходного тела. При sameShell != cm_Copy построенные тела нельзя перемещать относительно друг друга.
                         \en Whether to copy the source solid. Bbuilt bodies can not move relative to each other when sameShell != Vm_Copy. \~
  \param[in] surface - \ru Секущая поверхность.
                       \en A cutting plane. \~
  \param[in] names - \ru Именователь.
                     \en An object for naming the new objects. \~
...
//------------------------------------------------------------------------------
/** \brief \ru Cоздать симметричное тело относительно плоскости.
           \en Create a symmetric solid relative to a plane. \~
  \details \ru Cоздать симметричное тело относительно плоскости XY локальной системы координат. \n
...
//------------------------------------------------------------------------------
/** \brief \ru Cоздать зеркальную копию тела относительно плоскости.
           \en Create a mirror copy of a solid relative to a plane. \~
  \details \ru Cоздать зеркальную копию тела относительно плоскости XY локальной системы координат. \n
...
//------------------------------------------------------------------------------
/** \brief \ru Построить фаски ребeр тела.
           \en Create chamfers for edges of the solid. \~
  \details \ru Построить фаски указанных ребeр тела. \n
...
/** \brief \ru Создать тонкостенное тело исключением граней.
  ...
  \param[in] offDists - \ru Множество индивидуальных значений толщин (должен быть синхронен с массивом offFaces).
...
//------------------------------------------------------------------------------
/** \brief \ru Cоздать одно тело из присланных тел.
           \en Create a solid from the specified solids. \~ 
  \details \ru Cоздать тело с объединением или без объединения пересекающихся тел. \n
    Если флаг проверки пересечения checkIntersect == true, то выполняется проверка на пересечение тел 
    и булева операция объединения пересекающихся тел заданного множества в одно тело. В противном случае
    объединение тел заданного множества выполняется простым перекладыванием граней всех тел в одно новое тело. \n
	  Если флаг регулярности множества тел isArray == true, то тела множества расположены в узлах 
...
/** \brief \ru Cоздать одно тело из присланных тел.
           \en Create a solid from the specified solids. \~ 
  \details \ru Cоздать одно тело из присланных тел, не меняя их. \n
...
//------------------------------------------------------------------------------
/** \brief \ru Cоздать крепеж по трехмерной точке. НЕ ИСПОЛЬЗОВАТЬ ВНЕ ТЕСТОВОГО ПРИЛОЖЕНИЯ!!! ФУНКЦИЯ НАХОДИТСЯ В РАЗРАБОТКЕ!!!
           \en Create fastener using 3D point. \~ 
    \details \ru Cоздать крепеж по трехмерной точке. \n
             \en Create fastener using 3D point. \n \~
  ...
  \param[in]  names     - \ru Именователь новых ганей.
