...
public:
  ...
  bool        IsEqual( const MbRect &, double eps ) const;
  /// \ru Проверить габариты на равенство. \en Check bounding boxes for equality. 
  bool        IsEqual( const MbRect &, double xeps, double yeps ) const;
...
// ---
inline bool MbRect::IsEqual( const MbRect & other, double eps ) const {
...
// \ru Проверка равенства с другим прямоугольником \en Check for equality with another rectangle 
// ---
inline bool MbRect::IsEqual( const MbRect & other, double xeps, double yeps ) const {
