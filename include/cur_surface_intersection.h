...
*/
// ---
class MATH_CLASS MbSurfaceIntersectionCurve : public MbCurve3D, public MbNestSyncItem {
private :
  ...
  MbeCurveGlueType  glueType;  ///< \ru Тип кривой по топологии. \en A curve type by topology. 
...
public:
  ...
  virtual void    CalculateWire( const MbStepData & stepData, MbMesh & mesh ) const; // \ru Вычислить сетку. \en Calculate mesh. 
  ...
  virtual void    ChangeCarrier     ( MbSpaceItem & item, MbSpaceItem & init ); // \ru Изменить носитель. \en Change the carrier. 
  virtual bool    ChangeCarrierBorne( MbSpaceItem & item, MbSpaceItem & init, const MbMatrix & matr ); // \ru Изменение носимые элементы. \en Change a carrier elements. 
  ...
  virtual bool    IsShift( const MbSpaceItem &, MbVector3D &, bool & ) const;
  ...
  inline  void    GetFirstDer      ( double & t, MbVector3D & ) const;
          ...
          */
          bool    MergeCurves( const MbSurfaceIntersectionCurve & addCurve, bool toBegin, bool fromBegin, bool allowCntr,
                               const VERSION version, bool insertInterimPoints = true );
          ...
          */
          ...
          MbCurve * ChooseCurve( const MbSurface & surf, bool faceSense, bool curveSense ) const;
          /** \brief \ru Вычислить вектор сдвига кривой.
                     \en Calculate a vector of curve movement. \~
            \details \ru Вычислить двумерный вектор сдвига ориентированной двумерной кривой. \n
                     \en Calculate two-dimensional vector of oriented two-dimensional curve movement. \n \~ 
          */
          ...
          bool    ChangeCurve  ( MbCurve * oldCrv, MbCurve & newCrv );
          /// \ru Заменить поверхность. \en Replace surface. 
          bool    ChangeSurface( MbSurface & oldSrf, MbSurface & newSrf, bool faceSense, bool curveSense );
  ...
  virtual void    GetPointsByEvenLengthDelta( size_t n, SArray<MbCartPoint3D> & pnts ) const; // \ru Выдать n точек кривой с равными интервалами по длине дуги. \en Get n points of a curve with equally spaced by the arc length. 
