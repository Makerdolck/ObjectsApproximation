...
public :
  ...
  */
  ...
  /** \brief \ru Определить число узлов NURBS кривой, нужное для аппроксимации кривой с заданной точностью.
    ...
    \result \ru Построенная NURBS кривая или NULL при  неуспешном построении.
            \en The constructed NURBS curve or NULL in a case of failure. \~
  */
  virtual size_t  NurbsCurveMinPoints( const MbNurbsParameters & tParameters, double epsilon = METRIC_DELTA ) const; 
  ...
  */
  ...
  /** \brief \ru Сдвинуть параметр вдоль кривой.
    ...
    \param[in] curveDir - \ru Направление смещения. Если curveDir - неотрицателно, то смещение направлено в сторону увеличения параметра. 
  ...
  */
  ...
  /** \brief \ru Изменить носитель.
    ...
    \ingroup Curves_3D
  */
  virtual bool    ChangeCarrierBorne( MbSpaceItem & item, MbSpaceItem & init, const MbMatrix & matr ); // \ru Изменение носителя. \en Changing of carrier. 
  ...
  */
  ...
  /** \brief \ru Определить, близкb ли две кривые метрически.
  ...
  */
  ...
  /// \ru Являются ли кривая инверсно такой же? \en Whether an inversed curve is the same. 
          bool    IsInverseSame( const MbCurve3D & curve ) const;
