...
public:  
  ...
  void    AddArray( const RPArray<Type> & arr ) { SPArray<Type>::AddArraySimple( arr ); m_sort = false; }
...
inline Type* CSPArray<Type>::RemoveObj( Type *delObject, DelType del ) {
  PRECONDITION( SPArray<Type>::nowDeletedElem  == 0 ); // \ru Bременно, для отладки \en Temporarily, for debugging. 
